...
VynodeArr 0.4.6 supports Windows x64, Linux x64, Docker x86-64, and Unraid x86-64 from one repository and one pair of locked movie and television engine revisions.
...
| Docker x86-64 | Supported | `ghcr.io/minerport/vynodearr-unified:0.4.6` |
...
Install `VynodeArr-0.4.6-win-x64-setup.exe` from the GitHub release, then open `http://127.0.0.1:8686/`. The installer creates one Windows service and one tray controller. Uninstall stops the gateway and both engines before removing application files, while preserving `C:\ProgramData\VynodeArr` unless the user removes it explicitly.
