...
test('movie library review keeps Plex and VynodeArr lists independent while allowing TMDB rematches',async()=>{
  ...
  for(const value of ['Compare every movie location','plexItems.slice','vynodeItems.slice','scanItems.slice','filePaths.join','item.filePath','Use Plex TMDB ID','Search by TMDB ID','/api/discover/details/movie/${tmdbId}','Use this match','/api/media-match','MOVIE FOLDERS','matched','unmatched','lettersOnly','filenameMatchesTitle','filename-mismatch','ADD AN UNMATCHED MOVIE FOLDER','Add existing folder to VynodeArr','tmdb:${selectedPlex.tmdbId}','searchForMovie: false','Title match','Already in VynodeArr as','comparisonTitleKey','ComparisonBadge','VynodeArr','Folder','No match','RenamePreview','/api/media-files/rename?domain=movie','Rename & organize','Naming-standard changes queued','plex-missing','No Plex match','Filename mismatch','hasFilenameMismatch'])assert.ok(review.includes(value),value);
