...
test('Music storage uses the shared folder route and engine-style folder access',async()=>{
  ...
  assert.match(view,/Boolean\(settings\.downloadPath&&settings\.libraryRoot/);
});
...
test('poster overlay canvas remains full width inside the editor preview column',async()=>{
...
test('poster overlay editor can switch between exact production library cards and placement tools',async()=>{
  ...
  assert.match(preview,/currentTemplateId === template\.id/);
  assert.match(preview,/layer\.id\.startsWith\(`\$\{template\.id\}:`\)/);
  ...
  assert.match(css,/\.overlay-studio-grid\.overlay-workspace-templates,\.overlay-studio-grid\.overlay-workspace-assignments\{grid-template-columns:minmax\(0,1fr\)\}/);
