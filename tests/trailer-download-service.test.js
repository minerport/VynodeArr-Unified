...
test('trailer downloader reports yt-dlp and builds a bounded official trailer command',async()=>{const calls=[],service=new TrailerDownloadService({root:'/trailers',spawnImpl:fakeSpawn(calls)}),status=await service.status(),result=await service.download({url:'https://www.youtube.com/watch?v=abc',title:'A Movie',year:2026,domain:'movie',tmdbId:42});assert.equal(status.available,true);assert.equal(status.version,'2026.08.01');assert.equal(result.tmdbId,42);assert.match(result.folder,/A Movie \(2026\) \[tmdb-42\]$/);assert.deepEqual(calls[1].args.slice(0,4),['--no-playlist','--no-overwrites','--max-filesize','1G']);assert.ok(calls[1].args.includes('bv*[height<=1080]+ba/b[height<=1080]'));assert.equal(calls[1].args.at(-1),'https://www.youtube.com/watch?v=abc');});
