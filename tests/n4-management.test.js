...
test('native interaction workflows replace an upstream-shaped generic shell',async()=>{
  ...
  for(const workflow of ['wireLibraryImportReview','Select all shown','Unmatched only','Possible mismatches','Find match','run-import-lookup','scanNameParts','comparableTitle','import-mismatch','import-unmatched'])assert.ok(script.includes(workflow),workflow);
  ...
  for(const workflow of ['quality-range-track','data-control="range"','Save limits to engine',"`/api/manage/${domain.value}/qualityDefinitions/${payload.id}`","method:'PUT'"])assert.ok(script.includes(workflow),workflow);
  ...
  for(const workflow of ['Rename & organize','Rename selected','Retry organize','wireHistoryActions','/api/media-files/rename','renameMediaPreview','renamePlans','previewId','renameMediaSignature','This media changed after the rename preview','existingPath','newPath',"name:'RenameMovie'","name:'RenameSeries'",'moveFiles:true','libraryFolder'])assert.ok(script.includes(workflow)||apiSource.includes(workflow),workflow);
});
