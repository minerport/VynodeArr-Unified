...
test('native interaction workflows replace an upstream-shaped generic shell',async()=>{
  ...
  for(const workflow of ['wireLibraryImportReview','Select all shown','Unmatched only','Possible mismatches','Find match','run-import-lookup','scanNameParts','comparableTitle','import-mismatch','import-unmatched','importedMatch','Already imported match','selected match is already in the library'])assert.ok(script.includes(workflow),workflow);
