...
test('native interaction workflows replace an upstream-shaped generic shell',async()=>{
  ...
  for(const workflow of ['liveQueue','includeMovie:true','includeSeries:true','includeEpisode:true','sabnzbd','mode:\'queue\'','clientStatus','clientFilename','/api/activity/queue/live'])assert.ok(apiSource.includes(workflow));
