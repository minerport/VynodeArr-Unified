...
test('native interaction workflows replace an upstream-shaped generic shell',async()=>{
  const [html,script,apiSource,clientSource,librarySource,queueSource,wantedSource,releaseBrowser,stateSource,importAnalysisSource,importMonitorSource]=await Promise.all([
    ...
    readFile(new URL('../apps/web/client/src/wanted.tsx',import.meta.url),'utf8'),
    ...
    readFile(new URL('../apps/web/client/src/import-monitor.tsx',import.meta.url),'utf8')
  ]);
  for(const route of ['#add','#wanted','#queue','#service/root-folders','#system'])assert.match(html,new RegExp(route));
  for(const workflow of ['SeriesSearch','SeasonSearch','Search entire show','Search entire season'])assert.match(wantedSource,new RegExp(workflow));
  ...
  for(const control of ['Release, source, quality…','All statuses','All sources','All qualities','Showing {visible.length} of {items.length} releases','release-sort-direction','Engine import destination'])assert.ok(releaseBrowser.includes(control),control);
  ...
  const rematchSource=apiSource.slice(apiSource.indexOf('async function rematchMedia'),apiSource.indexOf('async function reassignMediaFile'));
  assert.match(rematchSource,/management\.execute\(domain, resource, method/,'manual matching must call the selected media engine through the management gateway');
  assert.doesNotMatch(rematchSource,/const execute = \(resource, method, options = \{\}\) =>\s*execute\(/,'the manual-match engine wrapper must not recursively call itself');
  ...
  for(const workflow of ['/api/storage/path-migration/preview','filesystemLocationIdentity','library_paths.migrated','moveFiles: false','Update at most 100 library locations at a time','input.final !== false','affectedCollections','collectionsUpdated','rootFolderPath: targetRoot'])assert.ok(apiSource.includes(workflow),'same-folder migrations must update titles and collection roots, remain bounded, and never move media files: '+workflow);
  for(const workflow of ['/api/storage/library-folder-children','Choose a folder inside the main /media mapping','segments.length > 9'])assert.ok(apiSource.includes(workflow),'nested media browsing must stay bounded inside /media: '+workflow);
...
assert.ok(apiSource.includes("domain==='movie'?{name:'RenameMovie',movieIds:[mediaId]}:{name:'RenameSeries',seriesIds:[mediaId]}"),'post-upgrade renaming must use the native movie and television rename commands');
  ...
  for(const workflow of ['/api/system/engine-updates','candidate-plan','PREPARE CANDIDATE','fresh_backup_required','engine_update.candidate_prepared'])assert.ok(apiSource.includes(workflow)||script.includes(workflow),workflow);
  ...
  for(const workflow of ["client.post('command',{name:'Restart'})",'eventSections(items)','wireEventFilters','event-toolbar'])assert.ok(script.includes(workflow)||apiSource.includes(workflow));
  ...
  for(const workflow of ['showAddMedia','discovery-art','remotePoster','showRootFolders','storage-config-grid','Download folder','Library folders','reviewMovieImport','reviewTvImport','reviewLibraryImport','Not imported','Already imported','imported-folder-group','vynodearr.dismissedImportJobs','startBackgroundImport','checkbox.checked=false','/api/import-jobs','Library imports','const target=event.currentTarget','Scan for','Import selected movies','Import selected series','showProfiles','showProviders','loadPolicy','Failed download handling','autoRedownloadFailed','Indexers','Download Clients','All provider options','folder-browser','Choose folder','Use this folder','attachDetailActions','episode-monitor','episode-auto-search','episode-interactive-search','Monitoring…','Unmonitoring…','Automatic search','Interactive search','release-table','data-sort','Source','Quality','Size','Seeders','grab-release','createRecord','Refresh & scan','Allowed qualities','Custom format scores','Create Movies and Television backups'])assert.match(script+importMonitorSource,new RegExp(workflow.replace(/[&]/g,'&')));
  assert.doesNotMatch(script,/button\.textContent='Importing…'/);
  for(const workflow of ['importIdentityKeys','skipped:job.skipped','already present/skipped','importPaceMs','VYNODEARR_IMPORT_PACE_MS','sync.invalidate(domain)','mapWithConcurrency(folders,4'])assert.ok(script.includes(workflow)||apiSource.includes(workflow)||importMonitorSource.includes(workflow),workflow);
  ...
  for(const workflow of ['televisionSeriesReleases','includeEpisodeFile:true','query.seriesId','query.seasonNumber','episode.seasonNumber','batchSize=8','interactiveReleaseCache','releaseCacheTtlMs=45_000','cachedInteractiveReleases','clearReleaseCache','reacquireRelease','mappedMovieId','mappedEpisodeInfo','episodeId','query.movieId','query.episodeId','no longer available from the search source','explainEmptyTelevisionSearch','No television indexer is enabled for interactive search','engineInstanceId: requestedInstance','release.indexerId ?? release.indexer','scope: "episodes"'])assert.ok(apiSource.includes(workflow),workflow);
  assert.ok(apiSource.includes('sync.reconcileItem(domain,publicId)'),'engine library edits must reconcile only the changed projection item');
  for(const workflow of ["route=movie?'movie':'series'",'freshRoots'])assert.ok(script.includes(workflow),workflow);
  assert.ok(wantedSource.includes('monitored=true'),'Wanted television queries retain monitored filtering');
  ...
  for(const workflow of ['attentionSnapshots','cachedAttention','refresh?await authoritativeAttention','Date.now() - lastLoadedAt >= 5 * 60 * 1000'])assert.ok(apiSource.includes(workflow)||librarySource.includes(workflow),workflow);
  assert.doesNotMatch(apiSource,/Promise\.all\(\[sync\.list\('movie',[\s\S]{0,160}authoritativeAttention\('movie'\)/,'ordinary movie navigation must not query live attention totals');
  assert.doesNotMatch(apiSource,/Promise\.all\(\[sync\.list\('tv',[\s\S]{0,160}authoritativeAttention\('tv'\)/,'ordinary television navigation must not query live attention totals');
