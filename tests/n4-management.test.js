...
test('native interaction workflows replace an upstream-shaped generic shell',async()=>{
  ...
  assert.ok(apiSource.includes("config/host',{...host,apiKey}"));
  assert.ok(!apiSource.includes("await restoreBundledCredentials();\n    if(!options.movie)"),'startup must not overwrite intentionally regenerated keys');
