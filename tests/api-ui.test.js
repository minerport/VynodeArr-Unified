...
async function fixtureServer(run){
  ...
  try{
    await fetch(`${base}/api/auth/setup`,{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({username:'reviewer',password:'correct-horse-review'})});
    const login=await fetch(`${base}/api/auth/login`,{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({username:'reviewer',password:'correct-horse-review'})});
    const loginBody=await login.json();const cookie=login.headers.get('set-cookie').split(';')[0];
    await run({base,cookie,csrf:loginBody.csrf});
  }finally{await new Promise((resolve)=>server.close(resolve));await rm(directory,{recursive:true,force:true});}
}
const get=(base,path,cookie)=>fetch(`${base}${path}`,{headers:{cookie}});

test('login, session validation, CSRF, and logout',()=>fixtureServer(async({base,cookie,csrf})=>{
...
test('UI exposes login, dashboard, media, operations, settings, and responsive shell',async()=>{
  ...
  for(const value of ['Sign in','Movies','TV','Queue','History','Calendar','Settings','System','Read-only review mode'])assert.match(html,new RegExp(value));
  for(const value of ['showDashboard','showMedia','showDetail','showOperational','showSettings'])assert.match(script,new RegExp(value));
