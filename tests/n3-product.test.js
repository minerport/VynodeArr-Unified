...
test('Reeltrack lists keep API keys server-side and match library titles only by durable external IDs',()=>{
  ...
  const reeltrackFetch=async(input,init={})=>{
    ...
    if(url.includes('/api/v1/lists/5/items'))return new Response(JSON.stringify({data:providerItemsState}),{status:200,headers:{'content-type':'application/json'}});
  ...
  const addedLibrary=[],movieRecords=[{id:1,title:'Durable match',tmdbId:1003598}],tvRecords=[{id:2,title:'TV match',tmdbId:7654,tvdbId:4567}],engineClient=(domain,records)=>({get:async(path,query)=>{if(path===(domain==='movie'?'movie':'series'))return records;if(path==='qualityprofile')return[{id:1,name:'Default'}];if(path==='rootfolder')return[{id:1,path:domain==='movie'?'/movies':'/tv'}];if(path===(domain==='movie'?'movie/lookup':'series/lookup')){const id=Number(String(query?.term||'').match(/\d+/)?.[0]);return[{title:`Engine ${id}`,tmdbId:id,...(domain==='tv'?{tvdbId:id}:{})}]};return[];},post:async(path,payload)=>{if(path===(domain==='movie'?'movie':'series')){const value={...payload,id:records.length+10};records.push(value);addedLibrary.push({domain,payload:value});return value}return{};}}),
    ...
    trailerCalls=[],collectionCalls=[],posterUploads=[],removeCalls=[],automationState={realArrived:false},trailerDownloader={status:async()=>({available:true,version:'test',root:'/movies',roots:{movie:'/movies',tv:'/tv'}}),download:async(input)=>{trailerCalls.push(input);const root=input.domain==='tv'?'/tv':'/movies';return{...input,path:`${root}/${input.tmdbId}/Trailer.mp4`,folder:`${root}/${input.tmdbId}`};},remove:async job=>{removeCalls.push(job);return true;}},
    detailCalls=[],discovery={configured:()=>false,setToken:()=>{},details:async(domain,id)=>{detailCalls.push({domain,id:Number(id)});return{title:`Trailer ${id}`,year:2026,poster:`https://image.tmdb.org/t/p/w500/${id}.jpg`,rating:8.4,genres:['Drama'],runtime:112,certification:'PG-13',studio:'Test Studio',trailer:{url:'https://youtube.com/watch?v=test'}}}},
    plexService={inspect:async()=>({endpoint:'http://plex.local:32400',server:{name:'Test Plex',machineIdentifier:'server-1'},libraries:[{key:'1',title:'Movies',type:'movie',uuid:'movies',locations:['/media/Movies']},{key:'2',title:'Shows',type:'show',uuid:'shows',locations:['/media/TV']},{key:'3',title:'Movie Placeholders',type:'movie',uuid:'movie-placeholders',locations:['/media/Movie Placeholders']},{key:'4',title:'TV Placeholders',type:'show',uuid:'tv-placeholders',locations:['/media/TV Placeholders']}]}),libraryItems:async(_endpoint,_token,library)=>{const domain=library.type==='show'?'tv':'movie',placeholderLibrary=['3','4'].includes(String(library.key)),root=String(library.locations?.[0]||(domain==='tv'?'/media/TV':'/media/Movies')),calls=trailerCalls.filter(item=>item.domain===domain),placeholders=calls.map((item,index)=>({ratingKey:`${library.key}${200+index}`,title:item.title||`Trailer ${item.tmdbId}`,type:library.type,thumb:`/library/metadata/${library.key}${200+index}/thumb`,files:[`${root}/${item.tmdbId}/Trailer.mp4`],Guid:[{id:`tmdb://${item.tmdbId}`}] }));if(placeholderLibrary)return placeholders;return automationState.realArrived?[...placeholders,...calls.map((item,index)=>({ratingKey:`${library.key}${800+index}`,title:`Real ${item.tmdbId}`,type:library.type,files:[`${root}/${item.tmdbId}/Real.mkv`],Guid:[{id:`tmdb://${item.tmdbId}`}] }))]:placeholders;},artwork:async()=>({body:Buffer.from('<svg xmlns="http://www.w3.org/2000/svg" width="600" height="900"><rect width="600" height="900" fill="#345678"/></svg>'),contentType:'image/png'}),refreshLibrary:async()=>true,uploadPoster:async(_endpoint,_token,ratingKey,body,type)=>{posterUploads.push({ratingKey,body,type});return true;},syncCollection:async(_endpoint,_token,input)=>{collectionCalls.push(input);return{ratingKey:`500${input.libraryKey}`,title:input.title,itemCount:input.ratingKeys.length};}};
  return appSession({movie,tv,fetcher:reeltrackFetch,trailerDownloader,discovery,plexService},async({base,cookie,csrf})=>{
    ...
    const poster=await fetch(`${base}/api/reeltrack/poster/movie/9999999`,{headers:{cookie},redirect:'manual'});assert.equal(poster.status,302,await poster.clone().text());assert.equal(poster.headers.get('location'),'https://image.tmdb.org/t/p/w500/9999999.jpg');
    const trailerStatus=await (await fetch(`${base}/api/reeltrack/trailers/status`,{headers:{cookie}})).json();assert.equal(trailerStatus.available,true);assert.equal(trailerStatus.engineRoots.movie[0].path,'/movies');assert.equal(trailerStatus.engineRoots.tv[0].path,'/tv');
    ...
    const plexConnection=await fetch(`${base}/api/poster-overlays/plex`,{method:'POST',headers:mutationHeaders,body:JSON.stringify({endpoint:'http://plex.local:32400',token:'plex-token'})});assert.equal(plexConnection.status,200);const rootConfirmation=await fetch(`${base}/api/reeltrack/trailers/root-folder`,{method:'POST',headers:mutationHeaders,body:JSON.stringify({domain:'movie',libraryKey:'1'})});assert.equal(rootConfirmation.status,201);
    const enabled=await fetch(`${base}/api/reeltrack/imported-lists/5/automation`,{method:'PUT',headers:mutationHeaders,body:JSON.stringify({enabled:true,plexMovieLibraryKey:'1',plexTvLibraryKey:'2',collectionName:'My watchlist',intervalMinutes:60,collectionPosterTemplate:{name:'Watchlist art',enabled:true,layers:[{kind:'shape',variable:'custom_text',shape:'square',width:100,height:100,x:0,y:0,background:'#08111f',backgroundOpacity:1}]},titleOverlayTemplate:{name:'Watchlist overlay',enabled:true,layers:[{kind:'text',variable:'title',width:80,x:10,y:70,background:'#08111f',backgroundOpacity:1}]}})});assert.equal(enabled.status,200);
    const automated=await fetch(`${base}/api/reeltrack/imported-lists/5/automation/run`,{method:'POST',headers:mutationHeaders}),automatedValue=await automated.json();assert.equal(automated.status,200,JSON.stringify(automatedValue));assert.equal(automatedValue.item.automation.status,'ready');assert.deepEqual(automatedValue.item.automation.plexLibraryLocations,{movie:'/media/Movies',tv:'/media/TV'});assert.ok((automatedValue.item.automation.summary.libraryAdded||0)+(automatedValue.item.automation.summary.libraryExisting||0)>=3,JSON.stringify(automatedValue.item.automation));assert.ok(addedLibrary.some(item=>item.domain==='movie'&&Number(item.payload.tmdbId)===9999999&&item.payload.addOptions.searchForMovie===false));assert.ok(Object.keys(automatedValue.item.automation.jobs||{}).length>=1,JSON.stringify(automatedValue.item.automation));assert.ok(collectionCalls.some(call=>call.libraryKey==='1'&&call.ratingKeys.length>=1));assert.ok(collectionCalls.some(call=>call.libraryKey==='2'&&call.ratingKeys.length>=1));assert.equal(collectionCalls.at(-1).title,'My watchlist');assert.ok(posterUploads.length>=2);assert.ok(posterUploads.every(item=>item.body.length>0));assert.ok(posterUploads.some(item=>item.type==='image/jpeg'));const artworkHistory=await (await fetch(`${base}/api/poster-overlays/plex/applications`,{headers:{cookie}})).json();assert.ok(artworkHistory.applications.some(item=>item.source==='list'&&item.listId==='5'&&item.role==='collection'));assert.ok(artworkHistory.applications.some(item=>item.source==='list'&&['placeholder','existing','newly_available'].includes(item.role)&&item.affectedCount>0));
    const uploadsBeforeReapply=posterUploads.length,detailsBeforeReapply=detailCalls.length,reapplied=await fetch(`${base}/api/reeltrack/imported-lists/5/automation/run`,{method:'POST',headers:mutationHeaders}),reappliedValue=await reapplied.json();assert.equal(reapplied.status,200,JSON.stringify(reappliedValue));assert.ok(posterUploads.slice(uploadsBeforeReapply).length>0);assert.ok(detailCalls.length>detailsBeforeReapply,'list overlays enrich Plex placeholders with provider metadata on every render');assert.ok(posterUploads.slice(uploadsBeforeReapply).every(item=>item.type==='image/jpeg'),'active collection and title updates upload only final composites');for(const ratingKey of new Set(posterUploads.map(item=>item.ratingKey)))assert.equal(posterUploads.filter(item=>item.ratingKey===ratingKey).at(-1).type,'image/jpeg',`${ratingKey} must finish with rendered artwork after restoring its original`);
    const removedPlaceholderKey='1200',uploadsBeforeListRemoval=posterUploads.length;providerItemsState=providerItemsState.filter(item=>Number(item.tmdbId)!==9999999);const listRemovalSync=await fetch(`${base}/api/reeltrack/imported-lists/5/automation/run`,{method:'POST',headers:mutationHeaders}),listRemovalValue=await listRemovalSync.json();assert.equal(listRemovalSync.status,200,JSON.stringify(listRemovalValue));assert.equal(posterUploads.slice(uploadsBeforeListRemoval).some(item=>String(item.ratingKey)===removedPlaceholderKey&&item.type==='image/png'),false,'a managed trailer placeholder keeps its overlay after leaving the provider list');providerItemsState=[...providerItemsState,{mediaId:124,title:'Durable match',type:'movie',year:2026,tmdbId:9999999,source:'tmdb',externalId:'9999999',rank:2}];
    const titleRestore=await fetch(`${base}/api/reeltrack/imported-lists/5/artwork/titles/restore`,{method:'POST',headers:mutationHeaders,body:'{}'}),titleRestoreValue=await titleRestore.json();assert.equal(titleRestore.status,200,JSON.stringify(titleRestoreValue));assert.ok(titleRestoreValue.restored>0);assert.equal(titleRestoreValue.item.automation.titleOverlayTemplate,null);assert.equal(posterUploads.at(-1).type,'image/png');
    const collectionCallsBeforeRestore=collectionCalls.length,collectionRestore=await fetch(`${base}/api/reeltrack/imported-lists/5/artwork/collection/restore`,{method:'POST',headers:mutationHeaders,body:'{}'}),collectionRestoreValue=await collectionRestore.json();assert.equal(collectionRestore.status,200,JSON.stringify(collectionRestoreValue));assert.equal(collectionRestoreValue.item.automation.collectionPosterTemplate,null);assert.ok(collectionRestoreValue.restored>0,'collection restore uploads the captured Plex artwork');assert.equal(posterUploads.at(-1).type,'image/png');assert.ok(collectionCalls.length>collectionCallsBeforeRestore,'collection membership remains synchronized after its artwork is restored');
    automationState.realArrived=true;const reconciled=await fetch(`${base}/api/reeltrack/imported-lists/5/automation/run`,{method:'POST',headers:mutationHeaders}),reconciledValue=await reconciled.json();assert.equal(reconciled.status,200,JSON.stringify(reconciledValue));assert.equal(removeCalls.length,0,'automatic sync must retain managed trailers');assert.ok(collectionCalls.at(-1).ratingKeys.length>=1,'real Plex media remains in the managed collection');
    const splitSaved=await fetch(`${base}/api/reeltrack/imported-lists/5/automation`,{method:'PUT',headers:mutationHeaders,body:JSON.stringify({enabled:true,splitLibraryMode:true,plexMovieLibraryKey:'1',plexTvLibraryKey:'2',plexMoviePlaceholderLibraryKey:'3',plexTvPlaceholderLibraryKey:'4',movieHostRoot:'/movies',tvHostRoot:'/tv',moviePlaceholderHostRoot:'/movies/placeholders',tvPlaceholderHostRoot:'/tv/placeholders',collectionName:'My watchlist',intervalMinutes:60})}),splitSavedValue=await splitSaved.json();assert.equal(splitSaved.status,200,JSON.stringify(splitSavedValue));const splitRun=await fetch(`${base}/api/reeltrack/imported-lists/5/automation/run`,{method:'POST',headers:mutationHeaders}),splitValue=await splitRun.json();assert.equal(splitRun.status,200,JSON.stringify(splitValue));assert.equal(splitValue.item.automation.splitLibraryMode,true);assert.equal(splitValue.item.automation.plexMoviePlaceholderLibraryKey,'3');assert.ok(collectionCalls.some(call=>String(call.libraryKey)==='3'),'split mode maintains a movie placeholder collection');assert.ok(collectionCalls.some(call=>String(call.libraryKey)==='1'),'split mode maintains a real movie collection');assert.ok(collectionCalls.some(call=>String(call.libraryKey)==='4'),'split mode maintains a TV placeholder collection');assert.ok(collectionCalls.some(call=>String(call.libraryKey)==='2'),'split mode maintains a real TV collection');
    providerItemsState=providerItemsState.filter(item=>item.type==='movie');const movieOnlySync=await fetch(`${base}/api/reeltrack/sync`,{method:'POST',headers:mutationHeaders});assert.equal(movieOnlySync.status,200);const movieOnlySave=await fetch(`${base}/api/reeltrack/imported-lists/5/automation`,{method:'PUT',headers:mutationHeaders,body:JSON.stringify({enabled:true,plexMovieLibraryKey:'1',movieHostRoot:'/movies',tvMediaDestinationId:'removed-tv-destination',collectionName:'Movies only',intervalMinutes:60})}),movieOnlyValue=await movieOnlySave.json();assert.equal(movieOnlySave.status,200,JSON.stringify(movieOnlyValue));assert.equal(movieOnlyValue.item.automation.tvMediaDestinationId,'','movie-only lists ignore stale television destinations');assert.equal(movieOnlyValue.item.automation.plexTvLibraryKey,'');
  });
});
...
test('poster overlays are opt-in, administrator-managed, and safely rendered',()=>appSession({
  ...
  const image='data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mNk+A8AAQUBAScY42YAAAAASUVORK5CYII=',uploaded=await fetch(`${base}/api/poster-overlays/assets`,{method:'POST',headers:{cookie,'content-type':'application/json','x-vynodearr-csrf':csrf},body:JSON.stringify({name:'Studio logo',image})}),asset=(await uploaded.json()).asset;assert.equal(uploaded.status,201);assert.match(asset.id,/^asset_/);assert.equal(asset.mime,'image/png');
  const assetConfiguration=await (await fetch(`${base}/api/poster-overlays`,{headers:{cookie}})).json();assert.ok(assetConfiguration.assets.some(item=>item.id===asset.id),JSON.stringify(assetConfiguration));
  const assetRead=await fetch(`${base}${asset.preview}`,{headers:{cookie}});assert.equal(assetRead.status,200,await assetRead.clone().text());assert.equal(assetRead.headers.get('content-type'),'image/png');assert.ok((await assetRead.arrayBuffer()).byteLength>0);
  const assetDeleted=await fetch(`${base}/api/poster-overlays/assets/${asset.id}`,{method:'DELETE',headers:{cookie,'x-vynodearr-csrf':csrf}});assert.equal(assetDeleted.status,200);assert.equal((await fetch(`${base}${asset.preview}`,{headers:{cookie}})).status,404);
  ...
  const namedAssignment=await fetch(`${base}/api/poster-overlays/assignments`,{method:'POST',headers:{cookie,'content-type':'application/json','x-vynodearr-csrf':csrf},body:JSON.stringify({name:'Quality badge — all movies',templateId:template.id,scope:{type:'all',domain:'movie'}})});assert.equal(namedAssignment.status,201);
  const renamed=await fetch(`${base}/api/poster-overlays/templates/${template.id}`,{method:'PUT',headers:{cookie,'content-type':'application/json','x-vynodearr-csrf':csrf},body:JSON.stringify({...template,name:'Added date'})});assert.equal(renamed.status,200);
  const overlayConfiguration=await (await fetch(`${base}/api/poster-overlays`,{headers:{cookie}})).json();assert.ok(overlayConfiguration.assignments.some(item=>item.templateId===template.id&&item.name==='Added date — all movies'),'renaming a template updates its generated assignment labels');
...
test('Plex poster connection previews, applies one title with rollback, and restores it',()=>{const uploads=[],original=Buffer.from('<svg xmlns="http://www.w3.org/2000/svg" width="600" height="900"><rect width="600" height="900" fill="#315c88"/></svg>');return appSession({movie:new MovieFixtureAdapter(),tv:new TvFixtureAdapter(),plexService:{inspect:async(endpoint,token)=>{assert.equal(endpoint,'http://plex.local:32400');assert.equal(token,'protected-token');return{endpoint,server:{name:'Review Plex',machineIdentifier:'plex-1',version:'1.41.0'},libraries:[{key:'1',title:'Movies',type:'movie',uuid:'movies-1'}]};},libraryItems:async()=>[{ratingKey:'91',title:'Orbit City',thumb:'/library/metadata/91/thumb/7',Guid:[{id:'tmdb://101'}]}],match:items=>items.map((item,index)=>({domain:item.domain,id:item.id,title:item.title,year:item.year,externalIds:index===0?['tmdb:101']:[],status:index===0?'matched':'unmatched',plex:index===0?[{ratingKey:'91',title:'Orbit City',year:2025,type:'movie',thumb:'/library/metadata/91/thumb/7'}]:[]})),artwork:async()=>({body:original,contentType:'image/svg+xml'}),uploadPoster:async(endpoint,token,ratingKey,value,contentType)=>{uploads.push({endpoint,token,ratingKey,value,contentType});}}},async({base,cookie,csrf})=>{
  ...
  const secondStyleResponse=await fetch(`${base}/api/poster-overlays/templates`,{method:'POST',headers:{cookie,'content-type':'application/json','x-vynodearr-csrf':csrf},body:JSON.stringify({name:'Second Plex layer',domain:'movie',target:'plex',layers:[{variable:'year',position:'bottom'}]})}),secondStyle=(await secondStyleResponse.json()).template;
  const secondApplied=await fetch(`${base}/api/poster-overlays/plex/apply`,{method:'POST',headers:{cookie,'content-type':'application/json','x-vynodearr-csrf':csrf},body:JSON.stringify({templateId:secondStyle.id,domain:'movie',mediaId:matched.id,libraryKey:'1',ratingKey:'91'})}),secondApplication=(await secondApplied.json()).application;assert.equal(secondApplied.status,201);assert.equal(uploads.length,2);assert.notDeepEqual(uploads[1].value,uploads[0].value,'the second assignment is composed over the first');
  const currentResponse=await fetch(`${base}/api/poster-overlays/plex/current/movie/${matched.id}?${params}`,{headers:{cookie}}),currentBytes=Buffer.from(await currentResponse.arrayBuffer());assert.equal(currentResponse.status,200);assert.deepEqual(currentBytes,uploads[1].value,'review current artwork uses the same complete composition as Plex');
  const secondRestored=await fetch(`${base}/api/poster-overlays/plex/applications/${secondApplication.id}/restore`,{method:'POST',headers:{cookie,'content-type':'application/json','x-vynodearr-csrf':csrf},body:'{}'});assert.equal(secondRestored.status,200);assert.equal(uploads.length,3);assert.deepEqual(uploads[2].value,uploads[0].value,'removing one assignment preserves every other active overlay');
  const restored=await fetch(`${base}/api/poster-overlays/plex/applications/${application.id}/restore`,{method:'POST',headers:{cookie,'content-type':'application/json','x-vynodearr-csrf':csrf},body:'{}'});assert.equal(restored.status,200);assert.equal((await restored.json()).application.status,'restored');assert.equal(uploads.length,4);assert.deepEqual(uploads[3].value,original);assert.equal(uploads[3].contentType,'image/svg+xml');
  const batchTarget={domain:'movie',mediaId:matched.id,title:matched.title,libraryKey:'1',ratingKey:'91'},batch=await fetch(`${base}/api/poster-overlays/plex/apply-batch`,{method:'POST',headers:{cookie,'content-type':'application/json','x-vynodearr-csrf':csrf},body:JSON.stringify({templateId:style.id,targets:[batchTarget]})}),batchValue=await batch.json();assert.equal(batch.status,200);assert.deepEqual(batchValue.summary,{requested:1,applied:1,failed:0});assert.equal(uploads.length,5);
  const updatedBatch=await fetch(`${base}/api/poster-overlays/plex/apply-batch`,{method:'POST',headers:{cookie,'content-type':'application/json','x-vynodearr-csrf':csrf},body:JSON.stringify({templateId:style.id,targets:[batchTarget]})});assert.equal(updatedBatch.status,200);assert.equal(uploads.length,6,'an overlay update sends only the final composite to Plex');assert.equal(uploads.at(-1).contentType,'image/jpeg');
  const duplicateBatch=await fetch(`${base}/api/poster-overlays/plex/apply-batch`,{method:'POST',headers:{cookie,'content-type':'application/json','x-vynodearr-csrf':csrf},body:JSON.stringify({templateId:style.id,targets:[batchTarget,batchTarget]})});assert.equal(duplicateBatch.status,400);assert.equal(uploads.length,6);
...
test('approval-required Discover requests stay out of the engine until an administrator approves them',()=>{
  ...
  return appSession({movie:Object.assign(new MovieFixtureAdapter(),{client:movieClient,listMovies:async()=>library}),tv:new TvFixtureAdapter(),discovery},async({base,cookie,csrf})=>{
    ...
    const clearedAdminNotification=await fetch(`${base}/api/notifications`,{method:'DELETE',headers:{cookie,'content-type':'application/json','x-vynodearr-csrf':csrf},body:JSON.stringify({ids:[adminNotifications.items[0].id]})});assert.equal(clearedAdminNotification.status,200);
    const afterAdminClear=await (await fetch(`${base}/api/notifications`,{headers:{cookie}})).json();assert.equal(afterAdminClear.items.some(item=>item.id===adminNotifications.items[0].id),false);
...
test('engine wizard validates actual read-only HTTP capabilities and saves only successful connections',async()=>{
  ...
  try{await appSession({movie:new MovieFixtureAdapter(),tv:new TvFixtureAdapter()},async({base,cookie,csrf})=>{
    ...
    for(const domain of ['movie','tv']){
      const tested=await fetch(`${base}/api/settings/engines/${domain}/test`,{method:'POST',headers:{cookie,'content-type':'application/json','x-vynodearr-csrf':csrf},body:JSON.stringify(input)}),testValue=await tested.json();assert.equal(testValue.validated,true);assert.ok(testValue.counts);assert.equal(testValue.checks.length,4);assert.ok(testValue.checks.every(check=>check.ok));
...
test('engine wizard identifies a failed capability without hiding a valid base connection',async()=>{
  const engine=createServer((req,res)=>{
    if(req.headers['x-api-key']!=='review-key'){res.writeHead(401);return res.end('{}');}
    const path=new URL(req.url,'http://engine').pathname;let value=[];
    if(path.endsWith('/system/status'))value={version:'5.0.0'};
    else if(path.endsWith('/movie'))value={unexpected:true};
    else if(path.endsWith('/queue'))value={records:[]};
    res.writeHead(200,{'content-type':'application/json'});res.end(JSON.stringify(value));
  });await new Promise((resolve)=>engine.listen(0,'127.0.0.1',resolve));const port=engine.address().port;
  try{await appSession({movie:new MovieFixtureAdapter(),tv:new TvFixtureAdapter()},async({base,cookie,csrf})=>{
    const response=await fetch(`${base}/api/settings/engines/instances/test`,{method:'POST',headers:{cookie,'content-type':'application/json','x-vynodearr-csrf':csrf},body:JSON.stringify({domain:'movie',host:'127.0.0.1',port,https:false,urlBase:'',apiCredential:'review-key',timeoutMs:1000,retries:0,tlsVerify:true})}),value=await response.json();
    assert.equal(response.status,200,JSON.stringify(value));assert.equal(value.connection.reachable,true);assert.equal(value.connection.authenticated,true);assert.equal(value.validated,false);
    assert.equal(value.checks.find(check=>check.key==='library').ok,false);assert.equal(value.checks.find(check=>check.key==='queue').ok,true);assert.doesNotMatch(JSON.stringify(value),/review-key/);
  });}finally{await new Promise((resolve)=>engine.close(resolve));}
});
test('external movie validation accepts a library when one Radarr record is incomplete',async()=>{
  const movies=[null,...Array.from({length:10001},(_,index)=>({id:index+1,title:`Valid movie ${index+1}`,year:2026,monitored:true}))];
  const engine=createServer((req,res)=>{const path=new URL(req.url,'http://engine').pathname;let value=[];if(req.headers['x-api-key']!=='review-key'){res.writeHead(401);return res.end('{}');}if(path.endsWith('/system/status'))value={version:'6.0.0'};else if(path.endsWith('/movie'))value=movies;else if(path.endsWith('/queue')||path.includes('/wanted/'))value={records:[]};res.writeHead(200,{'content-type':'application/json'});res.end(JSON.stringify(value));});
  await new Promise(resolve=>engine.listen(0,'127.0.0.1',resolve));
  try{await appSession({movie:new MovieFixtureAdapter(),tv:new TvFixtureAdapter()},async({base,cookie,csrf})=>{const response=await fetch(`${base}/api/settings/engines/instances/test`,{method:'POST',headers:{cookie,'content-type':'application/json','x-vynodearr-csrf':csrf},body:JSON.stringify({domain:'movie',host:'127.0.0.1',port:engine.address().port,https:false,urlBase:'',apiCredential:'review-key',timeoutMs:1000,retries:0,tlsVerify:true})}),value=await response.json();assert.equal(response.status,200);assert.equal(value.validated,true,JSON.stringify(value));assert.equal(value.counts.library,10001);});}finally{await new Promise(resolve=>engine.close(resolve));}
});
