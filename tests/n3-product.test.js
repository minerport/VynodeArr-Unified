...
test('Reeltrack lists keep API keys server-side and match library titles only by durable external IDs',()=>{
  ...
  return appSession({movie,tv,fetcher:reeltrackFetch,trailerDownloader,discovery,plexService},async({base,cookie,csrf})=>{
    ...
    const automated=await fetch(`${base}/api/reeltrack/imported-lists/5/automation/run`,{method:'POST',headers:mutationHeaders}),automatedValue=await automated.json();assert.equal(automated.status,200,JSON.stringify(automatedValue));assert.equal(automatedValue.item.automation.status,'ready');assert.deepEqual(automatedValue.item.automation.plexLibraryLocations,{movie:'/movies',tv:'/tv'});assert.ok((automatedValue.item.automation.summary.libraryAdded||0)+(automatedValue.item.automation.summary.libraryExisting||0)>=3,JSON.stringify(automatedValue.item.automation));assert.ok(addedLibrary.some(item=>item.domain==='movie'&&Number(item.payload.tmdbId)===9999999&&item.payload.addOptions.searchForMovie===false));assert.ok(Object.keys(automatedValue.item.automation.jobs||{}).length>=1,JSON.stringify(automatedValue.item.automation));assert.ok(collectionCalls.some(call=>call.libraryKey==='1'&&call.ratingKeys.length>=1));assert.ok(collectionCalls.some(call=>call.libraryKey==='2'&&call.ratingKeys.length>=1));assert.equal(collectionCalls.at(-1).title,'My watchlist');assert.equal(posterUploads.length,2);assert.ok(posterUploads.every(item=>item.type==='image/jpeg'&&item.body.length>0));
