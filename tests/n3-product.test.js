...
test('Reeltrack lists keep API keys server-side and match library titles only by durable external IDs',()=>{
  ...
  const movie=Object.assign(new MovieFixtureAdapter(),{listMovies:async()=>[{id:'movie_durable',title:'Durable match',year:2026,tmdbId:1003598,hasFile:true}]}),
    ...
    trailerCalls=[],collectionCalls=[],removeCalls=[],automationState={realArrived:false},trailerDownloader={status:async()=>({available:true,version:'test',root:'/movies'}),download:async(input)=>{trailerCalls.push(input);return{...input,path:`/movies/${input.tmdbId}/Trailer.mp4`,folder:`/movies/${input.tmdbId}`};},remove:async job=>{removeCalls.push(job);return true;}},
  ...
  return appSession({movie,tv,fetcher:reeltrackFetch,trailerDownloader,discovery,plexService},async({base,cookie,csrf})=>{
    ...
    const enabled=await fetch(`${base}/api/reeltrack/imported-lists/5/automation`,{method:'PUT',headers:mutationHeaders,body:JSON.stringify({enabled:true,plexLibraryKey:'1',plexTrailerPath:'/movies',collectionName:'My watchlist',intervalMinutes:60})});assert.equal(enabled.status,200);
