...
export class LibraryCatalogStore {
  ...
  async attentionSummary(domain,engineInstanceId='all'){await this.initialize();const scoped=engineInstanceId&&engineInstanceId!=='all',clause=scoped?"domain=? AND json_extract(payload_json,'$.engineInstanceId')=?":"domain=?",params=scoped?[domain,String(engineInstanceId)]:[domain],row=this.db().prepare(`SELECT coalesce(sum(CASE WHEN monitoring<>'none' THEN missing_count ELSE 0 END),0) missing,coalesce(sum(CASE WHEN monitoring<>'none' THEN cutoff_count ELSE 0 END),0) cutoff FROM library_items WHERE ${clause}`).get(...params);return{missing:Number(row.missing),cutoff:Number(row.cutoff)};}
  async librarySummary(domain,engineInstanceId='all'){await this.initialize();const scoped=engineInstanceId&&engineInstanceId!=='all',clause=scoped?"domain=? AND json_extract(payload_json,'$.engineInstanceId')=?":"domain=?",params=scoped?[domain,String(engineInstanceId)]:[domain],row=this.db().prepare(`SELECT count(*) total,coalesce(sum(CASE WHEN monitoring<>'none' THEN 1 ELSE 0 END),0) monitored,coalesce(sum(CASE WHEN (?='movie' AND has_file=1) OR (?='tv' AND missing_count=0) THEN 1 ELSE 0 END),0) covered FROM library_items WHERE ${clause}`).get(domain,domain,...params);return{total:Number(row.total),monitored:Number(row.monitored),covered:Number(row.covered)};}
  async queryDomain(domain,{offset=0,limit=5000,query='',filter='all',sort='title',direction='ascending',randomSeed=0,engineInstanceId='all'}={}){
    await this.initialize();const where=['domain=?'],params=[domain],term=String(query).trim().toLocaleLowerCase();if(term){where.push('(sort_title LIKE ? OR title LIKE ?)');params.push(`%${term}%`,`%${term}%`);}if(engineInstanceId&&engineInstanceId!=='all'){where.push("json_extract(payload_json,'$.engineInstanceId')=?");params.push(String(engineInstanceId));}if(filter==='monitored')where.push("monitoring<>'none'");if(filter==='unmonitored')where.push("monitoring='none'");if(filter==='missing')where.push("monitoring<>'none' AND missing_count>0");if(filter==='cutoff')where.push("monitoring<>'none' AND cutoff_count>0");
