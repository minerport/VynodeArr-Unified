...
export class LibraryCatalogStore {
  ...
  async applicationCatalogState(){await this.initialize();return this.getState('application:catalog-version',null);}
  async saveApplicationCatalogState(value){await this.initialize();return this.setState('application:catalog-version',value);}
  ...
  async queryDomain(domain,{offset=0,limit=5000,query='',filter='all',sort='title',direction='ascending',randomSeed=0}={}){
    ...
    const columns={title:'sort_title',year:'year',added:'updated_at',size:'json_extract(payload_json,\'$.sizeOnDisk\')',rating:'json_extract(payload_json,\'$.rating\')',quality:'json_extract(payload_json,\'$.quality\')'},column=columns[sort]||'sort_title',order=direction==='descending'?'DESC':'ASC',clamped=Math.max(1,Math.min(5000,Number(limit)||60)),start=Math.max(0,Number(offset)||0),clause=where.join(' AND '),total=Number(this.db().prepare(`SELECT count(*) count FROM library_items WHERE ${clause}`).get(...params).count),random=sort==='random',orderColumn=random?'abs((length(id)*1103515245 + unicode(substr(id,-1))*12345 + ?) % 2147483647)':column,orderParams=random?[Math.abs(Number(randomSeed)||0)%2147483647]:[],rows=this.db().prepare(`SELECT payload_json FROM library_items WHERE ${clause} ORDER BY ${orderColumn} ${order}, sort_title ASC, id ASC LIMIT ? OFFSET ?`).all(...params,...orderParams,clamped,start),groups=this.db().prepare(`SELECT CASE WHEN upper(substr(trim(title),1,1)) BETWEEN 'A' AND 'Z' THEN upper(substr(trim(title),1,1)) ELSE '#' END letter,count(*) count FROM library_items WHERE ${clause} GROUP BY letter ORDER BY CASE WHEN letter='#' THEN 0 ELSE 1 END,letter`).all(...params);let cursor=0;const letters={};for(const group of groups){letters[group.letter]={offset:cursor,count:Number(group.count)};cursor+=Number(group.count);}
