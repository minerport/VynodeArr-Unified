...
export class PlexService{
  ...
  async libraryCollections(endpoint,token,libraryKey){if(!/^\d+$/.test(String(libraryKey||'')))throw new Error('Plex library target is invalid');const response=await this.request(endpoint,token,`/library/sections/${libraryKey}/collections`),metadata=response.value?.MediaContainer?.Metadata,items=response.type==='json'?(Array.isArray(metadata)?metadata:metadata?[metadata]:[]):[...String(response.value).matchAll(/<Directory\b[^>]*(?:\/>|>[\s\S]*?<\/Directory>)/gi)].map(match=>match[0]);return items.map(item=>({ratingKey:String(item.ratingKey??xmlAttribute(item,'ratingKey')),title:decodeXml(item.title??xmlAttribute(item,'title')),smart:String(item.smart??xmlAttribute(item,'smart'))==='1'})).filter(item=>item.ratingKey);}
  async collectionItems(endpoint,token,ratingKey){if(!/^\d+$/.test(String(ratingKey||'')))throw new Error('Plex collection target is invalid');const response=await this.request(endpoint,token,`/library/metadata/${ratingKey}/children?includeGuids=1`);return plexMetadata(response).map(item=>item.ratingKey).filter(Boolean);}
  async syncCollection(endpoint,token,{libraryKey,libraryType,machineIdentifier,title,ratingKeys,replace=false}){if(!/^\d+$/.test(String(libraryKey||''))||!String(machineIdentifier||'').trim())throw new Error('Plex collection target is incomplete');const name=String(title||'').trim().slice(0,120);if(!name)throw new Error('Plex collection name is required');const matches=(await this.libraryCollections(endpoint,token,libraryKey)).filter(item=>item.title.toLowerCase()===name.toLowerCase()),ids=[...new Set((ratingKeys||[]).map(String).filter(value=>/^\d+$/.test(value)))];if(matches.some(item=>item.smart))throw new Error(`Plex collection “${name}” is smart and cannot be managed safely. Rename it or choose another collection name.`);if(replace){for(const item of matches)await this.command(endpoint,token,`/library/metadata/${encodeURIComponent(item.ratingKey)}`,{method:'DELETE'});}else if(matches.length){const collection=matches[0],current=new Set(await this.collectionItems(endpoint,token,collection.ratingKey)),missing=ids.filter(id=>!current.has(id));if(missing.length){const query=new URLSearchParams({uri:`server://${machineIdentifier}/com.plexapp.plugins.library/library/metadata/${missing.join(',')}`});await this.command(endpoint,token,`/library/collections/${collection.ratingKey}/items?${query}`,{method:'PUT'});}return{ratingKey:collection.ratingKey,title:name,itemCount:current.size+missing.length,added:missing.length,created:false};}if(!ids.length)return{ratingKey:null,title:name,itemCount:0,added:0,created:false};const query=new URLSearchParams({type:libraryType==='show'?'2':'1',title:name,smart:'0',sectionId:String(libraryKey),uri:`server://${machineIdentifier}/com.plexapp.plugins.library/library/metadata/${ids.join(',')}`});const created=await this.command(endpoint,token,`/library/collections?${query}`,{method:'POST'}),ratingKey=String(created.value?.MediaContainer?.Metadata?.[0]?.ratingKey||created.value?.MediaContainer?.Metadata?.ratingKey||created.location.match(/\/library\/metadata\/(\d+)/)?.[1]||'')||null;return{ratingKey,title:name,itemCount:ids.length,added:ids.length,created:true};}
