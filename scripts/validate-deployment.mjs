...
const unraid=await readFile(new URL('../infrastructure/unraid/vynodenew.xml',import.meta.url),'utf8');
const failures=[];
...
if(!image.includes('USER vynodenew'))failures.push('Production image does not use its unprivileged user');
if(!image.includes('HEALTHCHECK'))failures.push('Production image health check missing');
for(const marker of ['<Name>VynodeNew</Name>','Target="4310"','/mnt/user/appdata/vynodenew','API_CREDENTIAL_FILE'])if(!unraid.includes(marker))failures.push(`Unraid marker missing: ${marker}`);
