...
for(const target of ['/media','/movies-2','/movies-3','/tv-2','/tv-3'])if(!new RegExp(`<Config Name="[^"]+" Target="${target}" Default=""[^>]+Required="false"[^>]*><\\/Config>`).test(unraid))failures.push(`Optional Unraid placeholder is invalid: ${target}`);
...
for(const marker of ['<Registry>https://ghcr.io</Registry>','<MyIP/>','<License>Apache-2.0</License>','<ReadMe>https://raw.githubusercontent.com/minerport/VynodeArr-Unified/main/README.md</ReadMe>'])if(!unraid.includes(marker))failures.push(`Community Applications canonical field missing: ${marker}`);
