...
// Static overlay workspace layout formerly injected by the route now lives in
// the shared stylesheet. This moves existing bytes from JavaScript to CSS.
const overlayStaticLayoutMigrationAllowance={css:6_000};
...
// Overlay operations now share one guided workspace with a lightweight
// overview and focused Templates, Assignments, and Plex artwork views. The
...
// Advanced Overlay Studio editing adds bounded undo/redo history, multi-layer
// alignment and grouping, reusable variants, JSON import/export validation,
// local draft recovery, keyboard movement, accessibility checks, and an
// assignment-impact review. Keep that deliberate growth confined to the
// independently loaded Poster Overlays route and its editor-only styles.
const advancedOverlayStudioAllowance={route:17_000,css:3_500};
// Precision canvas controls keep zoom, guides, snapping, and keyboard movement
// orchestration in the overlay route while their control surface loads lazily.
const overlayPrecisionCanvasAllowance={route:300};
// Final Overlay Studio hardening adds reviewed conflict-aware imports, blocking
// design validation, real-title matrices, exact before/after review, and retry UX.
...
// and shared stylesheet instead of raising the base budget for every route.
const engineScopeWorkspaceAllowance={reeltrackRoute:1_300,css:2_400};
// Assignment review now renders the exact current and proposed poster inside
// every supported library-card view, grouped by library, while preserving the
// compact before/after summary. Keep this review-only growth isolated here.
const exactOverlayAssignmentReviewAllowance={route:400,css:3_400};
// Live History uses denser phone rows, while Custom Format scoring and template
// review use stacked controls, dynamic viewport scrolling, and safe-area-aware
// actions so the complete workflow remains reachable in mobile Safari.
const mobileHistoryAndFormatAllowance={css:1_800};
// Discover trailer playback adds a card action and one responsive, sound-enabled
// video modal while keeping trailer metadata lazy-loaded with the Discover route.
const discoverTrailerAllowance={css:2_200};
const failures=[];
...
for(const chunk of routeChunks){const routeLimit=limits.route+(chunk.name.startsWith('system-')?performanceAllowance.systemRoute:0)+(chunk.name.startsWith('poster-overlays-')?posterOverlayAllowance.route+overlayAssignmentClarityAllowance.route+overlayWorkspaceAllowance.route+advancedOverlayStudioAllowance.route+overlayPrecisionCanvasAllowance.route+overlayCompletionAllowance.route+exactOverlayAssignmentReviewAllowance.route:0)+(chunk.name.startsWith('root-folders-')?storageMappingAllowance.route+mediaDestinationChoiceAllowance.rootFoldersRoute+storagePathMigrationAllowance.route+storagePathProgressAllowance.route+storageEngineVerificationAllowance.route+nestedMediaFolderAllowance.route+multiEngineStorageAllowance.route:0)+(chunk.name.startsWith('reeltrack-lists-')?mediaDestinationChoiceAllowance.reeltrackRoute+mixedListSectionsAllowance.route+reeltrackRealTitleOverlayAllowance.route+reeltrackTrailerRepairAllowance.route+reeltrackDraftImportAllowance.route+reeltrackSplitLibraryAllowance.route+engineScopeWorkspaceAllowance.reeltrackRoute:0);if(chunk.bytes>routeLimit)failures.push(`${chunk.name} is ${chunk.bytes} bytes (route limit ${routeLimit}).`);}
if(stylesheet&&stylesheet.bytes>limits.css+mobileAllowance.css+performanceAllowance.css+posterOverlayAllowance.css+reeltrackAllowance.css+trailerDownloadAllowance.css+managedListCompactAllowance.css+artworkDesignerAllowance.css+libraryReviewAllowance.css+mediaRemovalAllowance.css+releaseBrowserAllowance.css+managedCollectionSetupAllowance.css+overlayAssignmentClarityAllowance.css+reeltrackApiKeyLinkAllowance.css+mobileActivityControlsAllowance.css+staticOverlayCssAllowance.css+overlayStaticLayoutMigrationAllowance.css+mixedListSectionsAllowance.css+reeltrackColumnOverscrollAllowance.css+mobileLibraryToolbarAllowance.css+mobileSetupOverviewAllowance.css+overlayWorkspaceAllowance.css+overlayItemPresetAllowance.css+advancedOverlayStudioAllowance.css+overlayCompletionAllowance.css+overlayIndependentColumnsAllowance.css+engineScopeWorkspaceAllowance.css+exactOverlayAssignmentReviewAllowance.css+mobileHistoryAndFormatAllowance.css+discoverTrailerAllowance.css)failures.push(`React stylesheet exceeds its intentional feature budget.`);
