...
const importJobs=new Map(),searchJobs=new Map(),namingAuditJobs=new Map(),completedQueueRefreshes=new Map(),completedQueueCleanups=new Map(),completedUpgradeRenames=new Map(),completedLibraryImports=new Map(),libraryReconciliations=new Map(),libraryEventClients=new Set(),interactiveReleaseCache=new Map(),renamePlans=new Map(),applicationBackupDownloads=new Map();
  ...
  async function reconcileSearchActivities(userId){
    ...
    const snapshots=new Map(await Promise.all(domains.map(async domain=>{const client=registry.get(domain).client,[queueValue,historyValue]=await Promise.all([client.get('queue',domain==='movie'?{page:1,pageSize:500,includeMovie:true}:{page:1,pageSize:500,includeSeries:true,includeEpisode:true}).catch(()=>({records:[]})),client.get('history',{page:1,pageSize:500,sortKey:'date',sortDirection:'descending'}).catch(()=>({records:[]}))]);return[domain,{queue:Array.isArray(queueValue?.records)?queueValue.records:[],history:Array.isArray(historyValue?.records)?historyValue.records:[]}];})));
