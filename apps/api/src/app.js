...
const importJobs=new Map(),searchJobs=new Map(),namingAuditJobs=new Map(),completedQueueRefreshes=new Map(),completedQueueCleanups=new Map(),completedUpgradeRenames=new Map(),completedLibraryImports=new Map(),libraryReconciliations=new Map(),libraryEventClients=new Set(),interactiveReleaseCache=new Map(),renamePlans=new Map();
  ...
  const notificationMessage=event=>`VynodeArr: ${event.title}\n${event.message}`;
  async function sendExternalNotification(channel,event){
    const credential=await engineSettings.notificationCredential(channel.id),message=notificationMessage(event);if(!credential)throw new Error('Credential is not configured');
    let response;if(channel.type==='discord'){if(!/^https:\/\/(?:[^/]+\.)?(?:discord\.com|discordapp\.com)\/api\/webhooks\//i.test(credential))throw new Error('Enter a valid Discord webhook URL');response=await fetch(credential,{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({content:message.slice(0,2000)}),signal:AbortSignal.timeout(10000)});}
    else if(channel.type==='telegram'){response=await fetch(`https://api.telegram.org/bot${credential}/sendMessage`,{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({chat_id:channel.chatId,text:message}),signal:AbortSignal.timeout(10000)});}
    else if(channel.type==='gotify'){const endpoint=String(channel.endpoint||'').replace(/\/+$/,'');if(!/^https?:\/\//i.test(endpoint))throw new Error('Enter a valid Gotify server URL');response=await fetch(`${endpoint}/message?token=${encodeURIComponent(credential)}`,{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({title:event.title,message:event.message,priority:event.severity==='critical'?8:event.severity==='warning'?5:2}),signal:AbortSignal.timeout(10000)});}else throw new Error('Unsupported notification provider');
