...
const importJobs=new Map(),searchJobs=new Map(),namingAuditJobs=new Map(),completedQueueRefreshes=new Map(),completedQueueCleanups=new Map(),completedUpgradeRenames=new Map(),completedLibraryImports=new Map(),libraryReconciliations=new Map(),libraryEventClients=new Set(),interactiveReleaseCache=new Map(),renamePlans=new Map(),applicationBackupDownloads=new Map();
  ...
  async function posterOverlayConfiguration(){const stored=await posterOverlayStore.read();return{templates:(stored.templates||[]).map(template=>({...template,layers:(template.layers||[]).map(sanitizeOverlayLayer)})),assignments:stored.assignments||[]};}
  ...
  async function renderedPlexPoster(target,template,session){
    const{item,context}=await overlayRenderContext(target.domain,target.item,template,session),svg=renderOverlaySvg({poster:target.poster.body,contentType:target.poster.contentType,template,item,context}),sharp=(await import('sharp')).default;
    const rendered=await sharp(Buffer.from(svg)).jpeg({quality:92,chromaSubsampling:'4:4:4'}).toBuffer();
