...
const importJobs=new Map(),searchJobs=new Map(),namingAuditJobs=new Map(),completedQueueRefreshes=new Map(),completedQueueCleanups=new Map(),completedUpgradeRenames=new Map(),completedLibraryImports=new Map(),libraryReconciliations=new Map(),libraryEventClients=new Set(),interactiveReleaseCache=new Map(),renamePlans=new Map(),applicationBackupDownloads=new Map();
  ...
  async function persistNotificationEvents(recipientUserId,events,{activeGroups=[]}={}){
    ...
    if(added.length)void deliverExternalNotifications(recipientUserId,added).catch(()=>{});
  }
  async function synchronizeOperationalNotifications(session){
    if(session.user.role!=='administrator')return;
    const stored=await notificationStore.read(),firstRun=!stored.operationalInitializedAt,since=stored.operationalInitializedAt||new Date().toISOString(),events=[],activeGroups=['queue-problem','engine-health','search-no-result'];
    ...
    for(const{domain,queue,history}of domainValues){
      ...
      if(!firstRun)for(const item of history){const timestamp=item.date||item.timestamp||item.createdAt;if(!timestamp||new Date(timestamp)<new Date(since))continue;const eventType=String(item.eventType||'').toLowerCase(),imported=/import/.test(eventType),failed=/fail/.test(eventType),grabbed=/grab/.test(eventType);if(!imported&&!failed&&!grabbed)continue;const title=item.movie?.title||item.series?.title||item.sourceTitle||item.title||`${domain==='movie'?'Movie':'Television'} item`,type=imported?'imported':failed?'failed':'grabbed';events.push({id:`operational:history:${domain}:${item.id||createHash('sha1').update(`${eventType}:${title}:${timestamp}`).digest('hex')}`,eventGroup:'history-event',category:imported?'import':'download',severity:imported?'success':failed?'critical':'information',type,title:imported?`${title} was imported`:failed?`${title} failed`:`${title} was grabbed`,message:imported?'The media engine confirmed this media is now in the library.':failed?friendlyRequestFailure(item.eventType||item.data?.message):'The media engine selected a release during background RSS or scheduled automation.',timestamp,href:failed?'#history':'#history',requestId:'',actionable:failed});}
    }
