...
import { aggregateOverlayFileMetadata,overlayRevision,posterVariableValues,posterVariables,renderOverlaySvg,resolveOverlayTemplate,sanitizeOverlayAssignment,sanitizeOverlayLayer,sanitizeOverlayTemplate } from '../../../packages/platform/src/poster-overlay-service.js';
...
export function createApplication(options={}){
  ...
  const plexSettingsStore=options.plexSettingsStore||new JsonStore(join(dataDir,'plex-settings.json'),{version:1,endpoint:'',server:null,libraries:[],updatedAt:null});
  const plexPosterApplicationStore=options.plexPosterApplicationStore||new JsonStore(join(dataDir,'plex-poster-applications.json'),{version:1,applications:[]});
  ...
  const notificationStore=options.notificationStore||new JsonStore(join(dataDir,'notification-events.json'),{version:1,events:[],reads:{}});
  ...
  const downloadDecisionStore=options.downloadDecisionStore||new JsonStore(join(dataDir,'download-decisions.json'),{version:1,decisions:[]});
  const operationsCenterStore=options.operationsCenterStore||new JsonStore(join(dataDir,'operations-center.json'),{version:1,dismissed:{}});
  ...
  const applicationBackupFiles=['users.json','engine-settings.json','credentials.enc','master-key','collections.json','poster-overlays.json','plex-settings.json','plex-poster-applications.json','user-requests.json','notification-events.json','guide-templates.json','engine-authentication.json','download-folders.json'];
  const applicationHistoryFiles=['search-activity.json','download-decisions.json','operations-center.json'];
  ...
  async function systemValidation(){
    const checks=[],managed=String(env.VYNODEARR_BUNDLED_ENGINES||'false')==='true',add=(id,group,title,status,message,action,details=[])=>checks.push({id,group,title,status,message,...(details.length?{details}:{}),...(action?{action}:{})});
    const reportedHealth=await sync.operations('health').catch(()=>[]);
    for(const domain of ['movie','tv']){
      const title=domain==='movie'?'Movies':'Television',connection=await registry.get(domain).testConnection().catch(error=>({reachable:false,authenticated:false,compatible:false,safeError:error instanceof Error?error.message:'Connection failed'}));
      const connected=connection.reachable&&connection.authenticated&&connection.compatible;
      add(`${domain}-connection`,'Media engines',`${title} engine`,connected?'healthy':'failed',connected?'Connection, authentication, and API compatibility passed.':connection.safeError||'The engine could not be reached and authenticated.',managed?{label:'Repair connection',repair:'engine-connections'}:{label:'Review engine settings',href:'#settings/engines'});
      if(!connected)continue;
      const [rootsResult,indexersResult,clientsResult,tasksResult,disksResult]=await Promise.allSettled(['rootFolders','indexers','downloadClients','tasks','diskSpace'].map(resource=>management.execute(domain,resource,'GET',{})));
      const value=result=>result.status==='fulfilled'&&Array.isArray(result.value)?result.value:[];
      const roots=value(rootsResult),indexers=value(indexersResult),clients=value(clientsResult),tasks=value(tasksResult),disks=value(disksResult),health=reportedHealth.filter(item=>item.domain===domain);
      const rootFailures=roots.filter(root=>root.accessible===false||Number(root.freeSpace)<0),lowSpace=disks.filter(disk=>Number(disk.totalSpace)>0&&Number(disk.freeSpace)<10*1024*1024*1024);
      add(`${domain}-storage`,'Storage & libraries',`${title} library paths`,rootsResult.status==='rejected'||!roots.length||rootFailures.length?'failed':lowSpace.length?'warning':'healthy',rootsResult.status==='rejected'?'Root folders could not be read.':!roots.length?'No root folder is configured.':rootFailures.length?'One or more root folders are unavailable.':lowSpace.length?'A library path has less than 10 GB free.':`${roots.length} root folder${roots.length===1?' is':'s are'} available.`,{label:'Review root folders',href:'#service/root-folders'},[...rootFailures.map(root=>`${root.path||'Root folder'} is unavailable`),...lowSpace.map(disk=>`${disk.path||'Storage path'} is low on space`)]);
      const enabledIndexers=indexers.filter(item=>item.enable!==false&&item.enableAutomaticSearch!==false),enabledClients=clients.filter(item=>item.enable!==false);
      add(`${domain}-indexers`,'Search & downloads',`${title} indexers`,indexersResult.status==='rejected'||!enabledIndexers.length?'failed':health.some(item=>/indexer|rss sync/i.test(`${item.source||''} ${item.message||''}`))?'warning':'healthy',indexersResult.status==='rejected'?'Indexer configuration could not be read.':!enabledIndexers.length?'No enabled automatic-search indexer is configured.':`${enabledIndexers.length} enabled indexer${enabledIndexers.length===1?' is':'s are'} configured.`,{label:'Review indexers',href:'#service/indexers'});
      add(`${domain}-clients`,'Search & downloads',`${title} download clients`,clientsResult.status==='rejected'||!enabledClients.length?'failed':health.some(item=>/download client/i.test(`${item.source||''} ${item.message||''}`))?'warning':'healthy',clientsResult.status==='rejected'?'Download-client configuration could not be read.':!enabledClients.length?'No enabled download client is configured.':`${enabledClients.length} enabled download client${enabledClients.length===1?' is':'s are'} configured.`,{label:'Review download clients',href:'#service/download-clients'});
      const unscheduled=tasks.filter(task=>!task.nextExecution);
      add(`${domain}-tasks`,'Automation',`${title} scheduled tasks`,tasksResult.status==='rejected'||!tasks.length?'failed':unscheduled.length?'warning':'healthy',tasksResult.status==='rejected'?'Scheduled tasks could not be read.':!tasks.length?'No engine tasks were reported.':unscheduled.length?`${unscheduled.length} task${unscheduled.length===1?' has':'s have'} no next execution time.`:`${tasks.length} recurring tasks are scheduled.`,{label:'Review tasks',href:'#system'});
      const severeHealth=health.filter(item=>/error|critical|failure/i.test(String(item.severity||item.type||'')));
      add(`${domain}-health`,'Media engines',`${title} reported health`,severeHealth.length?'failed':health.length?'warning':'healthy',health.length?`${health.length} engine warning${health.length===1?' requires':'s require'} review.`:'The engine reports no active health warnings.',{label:'Open Health',href:'#health'},health.slice(0,5).map(item=>String(item.message||'Engine warning')));
      const synchronization=sync.snapshot()[domain],syncHealthy=synchronization?.status==='ready'&&synchronization.lastSuccess;
      add(`${domain}-synchronization`,'Automation',`${title} library synchronization`,syncHealthy?'healthy':synchronization?.status==='stale'?'warning':'failed',syncHealthy?`Last completed ${synchronization.lastSuccess}.`:synchronization?.safeError||'No successful library synchronization is recorded.',{label:'Re-synchronize',repair:'synchronize'});
    }
    const stores=[['Users',()=>auth.listUsers()],['Requests',()=>requestStore.read()],['Collections',()=>collectionStore.read()],['Notifications',()=>notificationStore.read()]],storeResults=await Promise.allSettled(stores.map(([,read])=>read())),failedStores=storeResults.map((result,index)=>result.status==='rejected'?stores[index][0]:null).filter(Boolean);
    add('application-data','Application','Application data stores',failedStores.length?'failed':'healthy',failedStores.length?`${failedStores.join(', ')} data could not be read.`:'Users, requests, collections, and notification data are readable.',failedStores.length?{label:'Review backups',href:'#system'}:null);
    const keyStatus=masterKeyService.status();add('master-key','Security','Credential encryption',keyStatus.source?'healthy':'failed',keyStatus.managed?'The persistent application master key is available.':'The environment-managed master key is available.',{label:'Review security',href:'#system'});
    const notificationData=await notificationStore.read(),channels=(notificationData.channels||[]).filter(channel=>channel.enabled!==false),credentialResults=await Promise.all(channels.map(channel=>engineSettings.notificationCredential(channel.id))),missingCredentials=channels.filter((channel,index)=>!credentialResults[index]),failureCutoff=Date.now()-7*86400_000,recentFailures=(notificationData.deliveries||[]).filter(item=>item.status==='failed'&&new Date(item.timestamp).getTime()>=failureCutoff).slice(0,5);
    add('notifications','Notifications','External notification channels',missingCredentials.length?'failed':recentFailures.length?'warning':'healthy',missingCredentials.length?`${missingCredentials.length} enabled channel${missingCredentials.length===1?' is':'s are'} missing credentials.`:recentFailures.length?'Recent external deliveries failed. Review channel history before relying on alerts.':channels.length?`${channels.length} enabled channel${channels.length===1?' is':'s are'} configured with protected credentials.`:'No external channel is enabled; in-app notifications remain active.',null,[...missingCredentials.map(channel=>`${channel.name} is missing its protected credential`),...recentFailures.map(item=>`${item.channelName||'Channel'}: ${item.error||'delivery failed'}`)]);
    const summary={healthy:checks.filter(check=>check.status==='healthy').length,warning:checks.filter(check=>check.status==='warning').length,failed:checks.filter(check=>check.status==='failed').length},overall=summary.failed?'failed':summary.warning?'warning':'healthy';
    return{generatedAt:new Date().toISOString(),applicationVersion,overall,summary,checks};
  }
  ...
  const operationTime=value=>String(value?.timestamp||value?.updatedAt||value?.createdAt||value?.observedAt||value?.requestedAt||value?.date||new Date().toISOString());
  const operationDomain=value=>['movie','tv'].includes(value?.domain)?value.domain:null;
  const operationTitle=(value,fallback='System activity')=>String(value?.title||value?.target||value?.name||fallback);
  async function operationsTimeline(){
    const [activity,decisions,audit,requests,notifications,plex,queue,history]=await Promise.all([
      searchActivityStore.read().catch(()=>({activities:[]})),downloadDecisionStore.read().catch(()=>({decisions:[]})),auditStore.read().catch(()=>({entries:[]})),requestStore.read().catch(()=>({requests:[]})),notificationStore.read().catch(()=>({events:[],deliveries:[]})),plexPosterApplicationStore.read().catch(()=>({applications:[]})),(mode==='engine'?liveQueue():sync.operations('queue')).catch(()=>[]),sync.operations('history').catch(()=>[])
    ]),items=[];
    for(const value of activity.activities||[])items.push({id:`search:${value.id}`,source:'search',category:'automation',domain:operationDomain(value),title:operationTitle(value,'Media search'),summary:value.message||`Search is ${value.status||'recorded'}.`,status:value.status||'unknown',timestamp:operationTime(value),href:'#history',actor:value.origin||'VynodeArr'});
    for(const value of decisions.decisions||[])items.push({id:`decision:${value.id}`,source:'decision',category:'download',domain:operationDomain(value),title:operationTitle(value,'Release decision'),summary:(value.reasons||[])[0]||`${value.decision||'Decision'} · ${value.quality||'quality unavailable'}`,status:value.decision||'recorded',timestamp:operationTime(value),href:'#history',actor:value.origin||'Media engine'});
    for(const value of audit.entries||[])items.push({id:`audit:${value.id}`,source:'audit',category:value.category||'administration',domain:operationDomain(value),title:operationTitle(value,'Administrator action'),summary:value.summary||value.action||'Administrator action recorded.',status:'completed',timestamp:operationTime(value),href:'#system',actor:value.actorName||value.username||'Administrator'});
    for(const value of requests.requests||[])items.push({id:`request:${value.id}`,source:'request',category:'request',domain:operationDomain(value),title:operationTitle(value,'Media request'),summary:value.message||`Request is ${value.status||'recorded'}.`,status:value.status||'unknown',timestamp:operationTime(value),href:'#request-management',actor:value.requestedByName||value.username||'User'});
    for(const value of notifications.deliveries||[])items.push({id:`delivery:${value.id}`,source:'notification',category:'notification',domain:null,title:value.title||'External notification',summary:value.status==='failed'?(value.error||'Delivery failed.'):`Delivered through ${value.channelName||value.type||'an external channel'}.`,status:value.status||'recorded',timestamp:operationTime(value),href:'#system',actor:value.channelName||'Notification service'});
    for(const value of plex.applications||[])items.push({id:`plex:${value.id}`,source:'plex',category:'artwork',domain:operationDomain(value),title:operationTitle(value,'Plex poster'),summary:`Poster overlay ${value.status||'change'}${value.templateName?` · ${value.templateName}`:''}.`,status:value.status||'completed',timestamp:operationTime(value),href:'#service/poster-overlays',actor:'Plex artwork'});
    for(const value of queue||[])items.push({id:`queue:${value.domain||'media'}:${value.id}`,source:'queue',category:'download',domain:operationDomain(value),title:operationTitle(value,'Queued download'),summary:value.errorMessage||value.statusMessages?.[0]?.messages?.[0]||`Download is ${value.status||value.trackedDownloadStatus||'queued'}.`,status:value.status||value.trackedDownloadStatus||'queued',timestamp:operationTime(value),href:'#queue',actor:'Media engine'});
    for(const value of history||[])items.push({id:`history:${value.domain||'media'}:${value.id}`,source:'history',category:'library',domain:operationDomain(value),title:operationTitle(value,'Library event'),summary:value.details||value.context||String(value.eventType||'Engine activity recorded.'),status:value.eventType||'recorded',timestamp:operationTime(value),href:'#history',actor:'Media engine'});
    const unique=new Map();for(const item of items.sort((a,b)=>b.timestamp.localeCompare(a.timestamp)))if(!unique.has(item.id))unique.set(item.id,item);return[...unique.values()].slice(0,2500);
  }
  async function operationsActions(){
    const [timeline,validation,state]=await Promise.all([operationsTimeline(),systemValidation().catch(()=>({checks:[]})),operationsCenterStore.read()]),actions=[];
    const add=value=>actions.push({...value,dismissedAt:state.dismissed?.[value.id]||null});
    for(const check of validation.checks||[])if(check.status!=='healthy')add({id:`validation:${check.id}`,severity:check.status==='failed'?'critical':'warning',domain:null,title:check.title,what:check.message,why:(check.details||[])[0]||'A system validation check did not pass.',affected:check.group||'VynodeArr',recommended:check.action?.label||'Review the related settings.',href:check.action?.href||'#system',timestamp:validation.generatedAt||new Date().toISOString(),source:'validation'});
    for(const item of timeline){const status=String(item.status||'').toLowerCase();if(item.source==='notification'&&status==='failed')add({id:`action:${item.id}`,severity:'warning',domain:null,title:`Notification failed: ${item.title}`,what:item.summary,why:'The external provider did not accept or complete the delivery.',affected:item.actor,recommended:'Review the channel and retry the failed delivery.',href:'#system',timestamp:item.timestamp,source:item.source});else if(item.source==='search'&&['failed','completed'].includes(status))add({id:`action:${item.id}`,severity:status==='failed'?'critical':'warning',domain:item.domain,title:item.title,what:item.summary,why:status==='failed'?'The automated search reported a failure.':'The search completed without a release entering Queue or History.',affected:item.domain==='tv'?'Television library':'Movie library',recommended:'Review Search Activity and Download Decisions before searching again.',href:'#history',timestamp:item.timestamp,source:item.source});else if(item.source==='queue'&&/fail|error|warning/.test(status))add({id:`action:${item.id}`,severity:'critical',domain:item.domain,title:item.title,what:item.summary,why:'The media engine reports that this queued download needs attention.',affected:item.domain==='tv'?'Television download':'Movie download',recommended:'Open Queue to inspect, retry, or remove the download.',href:'#queue',timestamp:item.timestamp,source:item.source});else if(item.source==='request'&&/pending|awaiting/.test(status))add({id:`action:${item.id}`,severity:'information',domain:item.domain,title:`Approval needed: ${item.title}`,what:item.summary,why:'This user requires administrator approval before the title can be added.',affected:item.actor,recommended:'Review and approve or reject the request.',href:'#request-management',timestamp:item.timestamp,source:item.source});}
...
const importJobs=new Map(),searchJobs=new Map(),namingAuditJobs=new Map(),completedQueueRefreshes=new Map(),completedQueueCleanups=new Map(),completedUpgradeRenames=new Map(),completedLibraryImports=new Map(),libraryReconciliations=new Map(),libraryEventClients=new Set(),interactiveReleaseCache=new Map(),renamePlans=new Map(),applicationBackupDownloads=new Map();
  ...
  async function reconcileSearchActivities(userId,providedSnapshots=null){
    const stored=await searchActivityStore.read(),items=(stored.activities||[]).filter(item=>userId==null||item.userId===userId).slice(0,75),active=items.filter(item=>['queued','searching','grabbed','downloading','completed'].includes(item.status)&&Date.now()-new Date(item.createdAt).getTime()<7*24*60*60*1000),domains=[...new Set(active.map(item=>item.domain))];
    const snapshots=providedSnapshots||new Map(await Promise.all(domains.map(async domain=>{const client=registry.get(domain).client,[queueValue,historyValue]=await Promise.all([client.get('queue',domain==='movie'?{page:1,pageSize:500,includeMovie:true}:{page:1,pageSize:500,includeSeries:true,includeEpisode:true}).catch(()=>({records:[]})),client.get('history',{page:1,pageSize:500,sortKey:'date',sortDirection:'descending'}).catch(()=>({records:[]}))]);return[domain,{queue:Array.isArray(queueValue?.records)?queueValue.records:[],history:Array.isArray(historyValue?.records)?historyValue.records:[]}];})));
    const mediaMatches=(activity,value)=>{if(activity.domain==='movie')return Number(value.movieId||value.movie?.id)===Number(activity.movieId);const episodeId=Number(value.episodeId||value.episode?.id),seriesId=Number(value.seriesId||value.series?.id||value.episode?.seriesId);return activity.episodeIds?.length?activity.episodeIds.includes(episodeId):activity.seriesId&&seriesId===Number(activity.seriesId);},recent=(activity,value)=>{const date=new Date(value.date||value.timestamp||value.createdAt||0).getTime();return !date||date>=new Date(activity.createdAt).getTime()-60_000;};
    await Promise.all(active.map(async item=>{const snapshot=snapshots.get(item.domain)||{queue:[],history:[]},history=snapshot.history.filter(value=>mediaMatches(item,value)&&recent(item,value)),imported=history.find(value=>/import/.test(String(value.eventType||'').toLowerCase())),failed=history.find(value=>/fail/.test(String(value.eventType||'').toLowerCase())),queued=snapshot.queue.find(value=>mediaMatches(item,value)),grabbed=history.find(value=>/grab/.test(String(value.eventType||'').toLowerCase()));if(imported)return updateSearchActivity(item.id,{status:'imported',message:'The download finished and was imported into the library.',finishedAt:new Date().toISOString()});if(failed)return updateSearchActivity(item.id,{status:'failed',message:friendlyRequestFailure(failed.eventType||failed.data?.message),finishedAt:new Date().toISOString()});if(queued){const status=String(queued.status||queued.trackedDownloadStatus||queued.trackedDownloadState||'').toLowerCase(),complete=/complete/.test(status)&&Number(queued.sizeleft??queued.sizeLeft??1)<=0;return updateSearchActivity(item.id,{status:'downloading',message:complete?'Download completed; waiting for the media engine to import it.':'The selected release is downloading through the configured client.',finishedAt:null});}if(grabbed)await updateSearchActivity(item.id,{status:'grabbed',message:'A release was grabbed and sent to the download client.',finishedAt:null});else if(item.commandId)try{const command=await management.execute(item.domain,'commands','GET',{id:item.commandId}),raw=String(command?.status||command?.state||'').toLowerCase();if(/fail|abort/.test(raw))await updateSearchActivity(item.id,{status:'failed',message:command?.message||'The engine reported that this search failed.',finishedAt:new Date().toISOString()});else if(/complete/.test(raw))await updateSearchActivity(item.id,{status:'completed',message:'Search completed without a matching download currently in Queue or History.',finishedAt:new Date().toISOString()});else if(/start|run/.test(raw))await updateSearchActivity(item.id,{status:'searching',message:'The media engine is checking configured indexers.'});}catch{}}));
    const latest=await searchActivityStore.read();return(latest.activities||[]).filter(item=>userId==null||item.userId===userId).slice(0,75);
  }
  ...
  const releaseIdentity=release=>String(release.guid||release.downloadUrl||release.infoUrl||release.title||'').slice(0,500);
  async function recordDownloadDecisions(userId,domain,query,releases,{source='interactive',selected=null}={}){
    const now=new Date().toISOString(),mediaId=Number(query.movieId||query.episodeId||query.seriesId)||null,rows=(Array.isArray(releases)?releases:[]).slice(0,250).map(release=>{
      const nativeReasons=(release.rejections||[]).map(String).filter(Boolean),identity=releaseIdentity(release),chosen=selected&&identity===selected,accepted=chosen||eligibleRelease(release),quality=release.quality?.quality?.name||release.quality?.name||'Unknown',customFormatScore=Number(release.customFormatScore??release.customFormatScoreOffset??0),preferredWordScore=Number(release.preferredWordScore??0),seeders=Number.isFinite(Number(release.seeders))?Number(release.seeders):null,ageDays=Number.isFinite(Number(release.age))?Number(release.age):null,size=Number(release.size||0),upgradeRejected=nativeReasons.some(reason=>/upgrade|cutoff|not an improvement/i.test(reason));
  ...
  async function recordEngineDownloadDecisions(userId,domain,history){
    ...
    for(const item of values){
      ...
      const related=values.filter(candidate=>{const candidateTime=new Date(candidate.date||candidate.timestamp||0).getTime(),candidateDownloadId=String(candidate.downloadId||candidate.data?.downloadId||'');return candidate!==item&&nativeHistoryMediaId(domain,candidate)===mediaId&&(!downloadId||!candidateDownloadId||candidateDownloadId===downloadId)&&candidateTime>=grabTime&&candidateTime-grabTime<=48*60*60*1000;}).sort((left,right)=>new Date(left.date||left.timestamp||0).getTime()-new Date(right.date||right.timestamp||0).getTime()),imported=related.find(candidate=>/import/.test(String(candidate.eventType||'').toLowerCase())),deleted=related.find(candidate=>/filedeleted|file deleted|deleted/.test(String(candidate.eventType||'').toLowerCase())),replacementReason=String(deleted?.data?.reason||deleted?.reason||''),reportedUpgrade=nativeHistoryUpgrade(imported)||nativeHistoryUpgrade(item)||nativeHistoryUpgrade(deleted)||Boolean(deleted&&/upgrade|replace|quality/i.test(replacementReason)),upgrade=imported||reportedUpgrade?reportedUpgrade:null,previousQuality=deleted?nativeHistoryQuality(deleted):null,previousScore=deleted?nativeHistoryScore(deleted):null,reasons=[];
      ...
      else if(upgrade&&previousQuality&&previousQuality===incomingQuality&&previousScore!==null&&incomingScore===previousScore)reasons.push(`Same-quality replacement at ${incomingScore} custom-format points. Engine history confirms the replacement but does not retain the candidate-ranking reason.`);
      ...
      else if(!imported)reasons.push('Grab captured. Waiting for the engine import event to confirm whether this is a new file or an upgrade.');
      else reasons.push('The engine imported this as a new file; no replaced-file event was reported for this title.');
      if(replacementReason)reasons.push(`Engine replacement reason: ${replacementReason}.`);
  ...
  async function recordEngineSearchActivities(userId,domain,queue,history){
    const values=Array.isArray(history)?history:[],queued=Array.isArray(queue)?queue:[],rows=[];
    for(const item of values){
      const eventType=String(item.eventType||'').toLowerCase();if(!/grab/.test(eventType))continue;
      const timestamp=item.date||item.timestamp||item.createdAt||new Date().toISOString(),mediaId=nativeHistoryMediaId(domain,item),episodeId=Number(item.episodeId||item.episode?.id)||null,downloadId=String(item.downloadId||item.data?.downloadId||''),sourceTitle=String(item.sourceTitle||item.title||'Engine-selected release'),mediaTitle=String(item.movie?.title||item.series?.title||item.episode?.series?.title||sourceTitle),originEventId=`${domain}:${item.id||createHash('sha256').update(`${mediaId}:${downloadId}:${sourceTitle}:${timestamp}`).digest('hex').slice(0,24)}`;
      const grabbedAt=new Date(timestamp).getTime(),related=values.filter(candidate=>{const candidateTime=new Date(candidate.date||candidate.timestamp||0).getTime(),candidateDownloadId=String(candidate.downloadId||candidate.data?.downloadId||'');return candidate!==item&&nativeHistoryMediaId(domain,candidate)===mediaId&&(!downloadId||!candidateDownloadId||candidateDownloadId===downloadId)&&candidateTime>=grabbedAt&&candidateTime-grabbedAt<=48*60*60*1000;}),imported=related.find(candidate=>/import/.test(String(candidate.eventType||'').toLowerCase())),failed=related.find(candidate=>/fail/.test(String(candidate.eventType||'').toLowerCase())),queueItem=queued.find(candidate=>downloadId&&String(candidate.downloadId||candidate.id||'')===downloadId),status=imported?'imported':failed?'failed':queueItem?'downloading':'grabbed',finishedAt=imported||failed?new Date(imported?.date||failed?.date||timestamp).toISOString():null;
      rows.push({id:`activity_engine_${createHash('sha256').update(originEventId).digest('hex').slice(0,24)}`,userId,domain,source:'engine',scope:domain==='movie'?'movie':'episode',title:mediaTitle,movieId:domain==='movie'?mediaId:null,seriesId:domain==='tv'?mediaId:null,seasonNumber:Number.isFinite(Number(item.episode?.seasonNumber))?Number(item.episode.seasonNumber):null,episodeIds:episodeId?[episodeId]:[],commandId:null,status,message:imported?'The engine-selected download finished and was imported into the library.':failed?friendlyRequestFailure(failed.eventType||failed.data?.message):queueItem?'The engine-selected release is downloading through the configured client.':'The engine selected this release during background RSS or scheduled automation.',createdAt:new Date(timestamp).toISOString(),updatedAt:new Date(imported?.date||failed?.date||timestamp).toISOString(),finishedAt,selection:{title:sourceTitle,quality:nativeHistoryQuality(item),size:Number(item.data?.size||item.size||0)},counts:null,originEventId});
    }
    if(!rows.length)return[];await searchActivityStore.update(current=>{current.version=1;current.activities=Array.isArray(current.activities)?current.activities:[];for(const row of rows){const index=current.activities.findIndex(value=>value.originEventId===row.originEventId||value.id===row.id);if(index>=0)current.activities[index]={...current.activities[index],...row};else current.activities.unshift(row);}current.activities=current.activities.sort((left,right)=>String(right.createdAt).localeCompare(String(left.createdAt))).slice(0,250);return rows.length;});return rows;
  }
  ...
  async function persistNotificationEvents(recipientUserId,events,{activeGroups=[],suppressExternalIds=new Set()}={}){
    const now=new Date().toISOString(),activeIds=new Set(events.map(item=>item.id)),added=[];
    await notificationStore.update(current=>{current.version=1;current.events=Array.isArray(current.events)?current.events:[];for(const item of events){const existing=current.events.find(value=>value.recipientUserId===recipientUserId&&value.id===item.id);if(existing)Object.assign(existing,item,{updatedAt:now});else{const created={...item,recipientUserId,createdAt:now,updatedAt:now};current.events.push(created);added.push(created);}}for(const existing of current.events)if(existing.recipientUserId===recipientUserId&&existing.actionable&&activeGroups.includes(existing.eventGroup)&&!activeIds.has(existing.id))Object.assign(existing,{actionable:false,resolvedAt:now,updatedAt:now,message:`Resolved · ${existing.message}`});current.events=current.events.sort((left,right)=>String(right.timestamp).localeCompare(String(left.timestamp))).slice(0,2000);return events.length;});
    const deliverable=added.filter(item=>!suppressExternalIds.has(item.id));if(deliverable.length)void deliverExternalNotifications(recipientUserId,deliverable).catch(()=>{});
  }
  async function synchronizeOperationalNotifications(session){
    if(session.user.role!=='administrator')return;
    const stored=await notificationStore.read(),cycleStartedAt=new Date().toISOString(),firstRun=!stored.operationalInitializedAt,since=stored.operationalInitializedAt||cycleStartedAt,grabDeliveryInitialized=Boolean(stored.operationalGrabDeliveryInitializedAt),events=[],suppressExternalIds=new Set(),activeGroups=['queue-problem','engine-health','search-no-result'];
    ...
    for(const{domain,queue,history}of domainValues){
      ...
      await recordEngineSearchActivities(session.user.id,domain,queue,history);
      ...
      for(const item of history){const timestamp=item.date||item.timestamp||item.createdAt;if(!timestamp)continue;const eventType=String(item.eventType||'').toLowerCase(),imported=/import/.test(eventType),failed=/fail/.test(eventType),grabbed=/grab/.test(eventType);if(!imported&&!failed&&!grabbed)continue;if(!grabbed&&(firstRun||new Date(timestamp)<new Date(since)))continue;if(grabbed&&grabDeliveryInitialized&&new Date(timestamp)<new Date(since))continue;const title=item.movie?.title||item.series?.title||item.sourceTitle||item.title||`${domain==='movie'?'Movie':'Television'} item`,type=imported?'imported':failed?'failed':'grabbed',id=`operational:history:${domain}:${item.id||createHash('sha1').update(`${eventType}:${title}:${timestamp}`).digest('hex')}`;events.push({id,eventGroup:'history-event',category:imported?'import':'download',severity:imported?'success':failed?'critical':'information',type,title:imported?`${title} was imported`:failed?`${title} failed`:`${title} was grabbed`,message:imported?'The media engine confirmed this media is now in the library.':failed?friendlyRequestFailure(item.eventType||item.data?.message):'The media engine selected a release during background RSS or scheduled automation.',timestamp,href:'#history',requestId:'',actionable:failed});if(grabbed&&!grabDeliveryInitialized)suppressExternalIds.add(id);}
    }
    ...
    await persistNotificationEvents(session.user.id,events,{activeGroups,suppressExternalIds});
    await notificationStore.update(current=>{current.operationalInitializedAt=cycleStartedAt;current.operationalGrabDeliveryInitializedAt=current.operationalGrabDeliveryInitializedAt||cycleStartedAt;return cycleStartedAt;});
  }
  ...
  async function requestNotifications(session){
    ...
    if(administratorUser){
      ...
      for(const record of records.filter(item=>item.status==='pending_approval'||item.approvedBy||item.rejectedBy)){
        const user=users.get(record.userId),name=user?.name||user?.username||'A user';
        const pending=record.status==='pending_approval',approved=Boolean(record.approvedBy);
        items.push({id:`approval:${record.id}`,category:'request',severity:pending?'warning':approved?'success':'information',type:pending?'approval':approved?'approved':'rejected',title:pending?`${name} requested ${record.title}`:`${record.title} was ${approved?'approved':'declined'}`,message:pending?`Review and decide whether to add this ${record.domain==='movie'?'movie':'television series'}.`:`${name}'s request was ${approved?'approved and added to the media engine':'declined'}.`,timestamp:record.requestedAt,href:'#request-management',requestId:record.id,actionable:pending});
      }
    }else{
      for(const record of records){
        if(record.approvedBy)items.push({id:`approved:${record.id}`,category:'request',severity:'success',type:'approved',title:`${record.title} was approved`,message:'An administrator approved your request and added it to the media engine.',timestamp:record.approvedAt||record.updatedAt||record.requestedAt,href:'#requests',requestId:record.id,actionable:false});
        if(record.status==='rejected')items.push({id:`rejected:${record.id}`,category:'request',severity:'information',type:'rejected',title:`${record.title} was declined`,message:record.rejectionReason||record.message||'An administrator declined this request.',timestamp:record.rejectedAt||record.updatedAt||record.requestedAt,href:'#requests',requestId:record.id,actionable:false});
        if(record.status==='failed')items.push({id:`failed:${record.id}`,category:'request',severity:'critical',type:'failed',title:`${record.title} needs attention`,message:record.message||'The media engine reported a problem with this request.',timestamp:record.updatedAt||record.requestedAt,href:'#requests',requestId:record.id,actionable:true});
        if(record.status==='imported')items.push({id:`imported:${record.id}`,category:'request',severity:'success',type:'imported',title:`${record.title} is ready`,message:'Your requested title has finished importing into the library.',timestamp:record.updatedAt||record.requestedAt,href:'#requests',requestId:record.id,actionable:false});
      }
    }
    await persistNotificationEvents(session.user.id,items);
    const [stored,legacy]=await Promise.all([notificationStore.read(),requestStore.read()]),reads={...(legacy.notificationReads?.[session.user.id]||{}),...(stored.reads?.[session.user.id]||{})};
    return(stored.events||[]).filter(item=>item.recipientUserId===session.user.id).sort((left,right)=>String(right.timestamp).localeCompare(String(left.timestamp))).slice(0,100).map(({recipientUserId,...item})=>({...item,read:Boolean(reads[item.id])}));
  }
  const notificationPreferenceDefaults={inApp:true,categories:{request:true,download:true,import:true,system:true,security:true},minimumSeverity:'information',quietHours:{enabled:false,start:22,end:7}};
  const sanitizeNotificationPreferences=value=>{const source=value&&typeof value==='object'?value:{},categories=source.categories&&typeof source.categories==='object'?source.categories:{},quiet=source.quietHours&&typeof source.quietHours==='object'?source.quietHours:{};return{inApp:source.inApp!==false,categories:Object.fromEntries(Object.keys(notificationPreferenceDefaults.categories).map(key=>[key,categories[key]!==false])),minimumSeverity:['information','warning','critical'].includes(source.minimumSeverity)?source.minimumSeverity:'information',quietHours:{enabled:Boolean(quiet.enabled),start:Math.min(23,Math.max(0,Number(quiet.start)||0)),end:Math.min(23,Math.max(0,Number(quiet.end)||0))}};};
  async function notificationPreferences(userId){const stored=await notificationStore.read(),defaults=sanitizeNotificationPreferences(stored.preferences?.defaults||notificationPreferenceDefaults),user=stored.preferences?.users?.[userId];return{defaults,preferences:sanitizeNotificationPreferences(user?{...defaults,...user,categories:{...defaults.categories,...user.categories},quietHours:{...defaults.quietHours,...user.quietHours}}:defaults),overridden:Boolean(user)};}
  const notificationQuietNow=preferences=>{if(!preferences.quietHours.enabled)return false;const hour=new Date().getUTCHours(),{start,end}=preferences.quietHours;return start===end||start<end?hour>=start&&hour<end:hour>=start||hour<end;};
  const filterNotifications=(items,preferences)=>{if(!preferences.inApp)return[];const rank={information:0,success:0,warning:1,critical:2},minimum=rank[preferences.minimumSeverity]||0;return items.filter(item=>preferences.categories[item.category||'system']!==false&&(rank[item.severity||'information']||0)>=minimum);};
  const defaultChannelTemplate={title:'{title}',message:'{message}',includeLink:true,accentColor:'#7c5cff',priority:5,json:''};
  const sanitizeChannelTemplate=value=>{const source=value&&typeof value==='object'?value:{},priority=Number(source.priority);return{title:String(source.title||defaultChannelTemplate.title).slice(0,300),message:String(source.message||defaultChannelTemplate.message).slice(0,3000),includeLink:source.includeLink!==false,accentColor:/^#[0-9a-f]{6}$/i.test(String(source.accentColor||''))?String(source.accentColor):defaultChannelTemplate.accentColor,priority:Number.isFinite(priority)?Math.min(10,Math.max(0,priority)):defaultChannelTemplate.priority,json:String(source.json||'').slice(0,12000)};};
  const notificationTokens=event=>({title:String(event.title||''),message:String(event.message||''),category:String(event.category||'activity'),severity:String(event.severity||'information'),timestamp:String(event.timestamp||new Date().toISOString()),link:String(event.href||'')});
  const renderNotificationText=(value,event)=>{const tokens=notificationTokens(event);return String(value||'').replace(/\{(title|message|category|severity|timestamp|link)\}/g,(_,key)=>tokens[key]);};
  function renderNotificationJson(value,event,depth=0){if(depth>8)throw new Error('Custom notification JSON is too deeply nested');if(Array.isArray(value))return value.slice(0,100).map(item=>renderNotificationJson(item,event,depth+1));if(value&&typeof value==='object')return Object.fromEntries(Object.entries(value).filter(([key])=>!['__proto__','prototype','constructor'].includes(key)).slice(0,100).map(([key,item])=>[key,renderNotificationJson(item,event,depth+1)]));return typeof value==='string'?renderNotificationText(value,event):value;}
  const channelPayload=(channel,event)=>{const template=sanitizeChannelTemplate(channel.template),link=template.includeLink&&event.href?`\n${event.href}`:'',title=renderNotificationText(template.title,event),message=`${renderNotificationText(template.message,event)}${link}`;if(template.json){let custom;try{custom=JSON.parse(template.json);}catch{throw new Error('Custom notification JSON is invalid');}if(!custom||Array.isArray(custom)||typeof custom!=='object')throw new Error('Custom notification JSON must be an object');return{title,message,priority:template.priority,payload:renderNotificationJson(custom,event)};}return{title,message,priority:template.priority,payload:null};};
  const pushoverCredential=value=>{try{const parsed=JSON.parse(value);return{token:String(parsed.token||''),userKey:String(parsed.userKey||''),encryptionKey:String(parsed.encryptionKey||'')};}catch{return{token:String(value||''),userKey:'',encryptionKey:''};}};
  const encryptPushoverField=async(value,keyHex)=>{const key=Buffer.from(keyHex,'hex'),compressed=await gzipAsync(Buffer.from(String(value||''),'utf8')),iv=randomBytes(16),cipher=createCipheriv('aes-256-cbc',key,iv),encrypted=Buffer.concat([cipher.update(compressed),cipher.final()]),signed=Buffer.concat([iv,encrypted]),mac=createHmac('sha256',key).update(signed).digest();return Buffer.concat([signed,mac]).toString('base64');};
  async function sendExternalNotification(channel,event){
    const credential=await engineSettings.notificationCredential(channel.id),rendered=channelPayload(channel,event);if(!credential)throw new Error('Credential is not configured');
    let response;if(channel.type==='discord'){if(!/^https:\/\/(?:[^/]+\.)?(?:discord\.com|discordapp\.com)\/api\/webhooks\//i.test(credential))throw new Error('Enter a valid Discord webhook URL');const color=Number.parseInt(sanitizeChannelTemplate(channel.template).accentColor.slice(1),16);response=await fetch(credential,{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify(rendered.payload||{embeds:[{title:rendered.title.slice(0,256),description:rendered.message.slice(0,4096),color}]}),signal:AbortSignal.timeout(10000)});}
    else if(channel.type==='telegram'){response=await fetch(`https://api.telegram.org/bot${credential}/sendMessage`,{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify(rendered.payload?{...rendered.payload,chat_id:channel.chatId}:{chat_id:channel.chatId,text:`${rendered.title}\n${rendered.message}`}),signal:AbortSignal.timeout(10000)});}
    else if(channel.type==='gotify'){const endpoint=String(channel.endpoint||'').replace(/\/+$/,'');if(!/^https?:\/\//i.test(endpoint))throw new Error('Enter a valid Gotify server URL');response=await fetch(`${endpoint}/message?token=${encodeURIComponent(credential)}`,{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify(rendered.payload||{title:rendered.title,message:rendered.message,priority:rendered.priority}),signal:AbortSignal.timeout(10000)});}
    else if(channel.type==='pushover'){const secret=pushoverCredential(credential);if(!secret.token||!secret.userKey)throw new Error('Pushover application token and user key are required');let title=rendered.title,message=rendered.message;const form=new URLSearchParams({token:secret.token,user:secret.userKey,title,message,priority:String(channel.pushoverPriority??0)});if(secret.encryptionKey){title=await encryptPushoverField(title,secret.encryptionKey);message=await encryptPushoverField(message,secret.encryptionKey);form.set('title',title);form.set('message',message);form.set('encrypted','1');}if(channel.devices?.length)form.set('device',channel.devices.join(','));if(Number(channel.pushoverPriority)===2){form.set('retry',String(channel.retry||60));form.set('expire',String(channel.expire||3600));}if(Number(channel.ttl)>0)form.set('ttl',String(channel.ttl));if(channel.sound)form.set('sound',channel.sound);response=await fetch('https://api.pushover.net/1/messages.json',{method:'POST',headers:{'content-type':'application/x-www-form-urlencoded'},body:form,signal:AbortSignal.timeout(10000)});}else throw new Error('Unsupported notification provider');
    if(!response.ok)throw new Error(`${channel.type} returned HTTP ${response.status}`);return true;
  }
  async function recordExternalDelivery(channel,event,status,error='',attempt=1){const value={id:`delivery_${randomUUID()}`,channelId:channel.id,channelName:channel.name,type:channel.type,eventId:event.id,title:event.title,status,error:String(error||'').slice(0,300),attempt,timestamp:new Date().toISOString()};await notificationStore.update(current=>{current.deliveries=[value,...(current.deliveries||[])].slice(0,200);});return value;}
  async function deliverExternalNotifications(recipientUserId,events){
    const stored=await notificationStore.read(),channels=(stored.channels||[]).filter(item=>item.enabled!==false),preferenceValue=await notificationPreferences(recipientUserId);if(notificationQuietNow(preferenceValue.preferences))return;
    for(const event of filterNotifications(events,{...preferenceValue.preferences,inApp:true}))for(const channel of channels)if((channel.categories||[]).includes(event.category||'system'))try{await sendExternalNotification(channel,event);await recordExternalDelivery(channel,event,'delivered');}catch(error){await recordExternalDelivery(channel,event,'failed',error instanceof Error?error.message:String(error));}
  }
  ...
  const collectionMediaId=(domain,value)=>String(value??'').replace(domain==='movie'?/^movie_/:/^series_/,'');
  const resolveCollectionMedia=(domain,library,candidate={})=>{
    const mediaId=collectionMediaId(domain,candidate.engineId??candidate.mediaId),tmdbId=String(candidate.tmdbId||''),tvdbId=String(candidate.tvdbId||''),title=String(candidate.title||'').trim().toLowerCase(),year=Number(candidate.year||0);
    const external=domain==='movie'?(tmdbId&&library.find(item=>String(item.tmdbId||'')===tmdbId)):(tvdbId&&library.find(item=>String(item.tvdbId||'')===tvdbId))||(tmdbId&&library.find(item=>String(item.tmdbId||'')===tmdbId));
    return external||(mediaId&&library.find(item=>collectionMediaId(domain,item.id)===mediaId))||library.find(item=>title&&String(item.title||'').trim().toLowerCase()===title&&(!year||Number(item.year||0)===year));
  };
  const sameCollectionMedia=(domain,left,right)=>domain==='movie'&&left.tmdbId&&right.tmdbId?String(left.tmdbId)===String(right.tmdbId):domain==='tv'&&left.tvdbId&&right.tvdbId?String(left.tvdbId)===String(right.tvdbId):domain==='tv'&&left.tmdbId&&right.tmdbId?String(left.tmdbId)===String(right.tmdbId):collectionMediaId(domain,left.engineId??left.mediaId)===collectionMediaId(domain,right.engineId??right.mediaId);
  async function userRequestCollections(session){
    const [stored,users,movies,television]=await Promise.all([requestStore.read(),auth.listUsers(),sync.list('movie'),sync.list('tv')]),userMap=new Map(users.map(user=>[user.id,user])),libraries={movie:movies,tv:television},preferences=stored.collectionPreferences||{},allUserIds=new Set([...users.map(user=>user.id),...(stored.requests||[]).map(item=>item.userId),...(stored.interests||[]).map(item=>item.userId)]),visibleUserIds=session.user.role==='administrator'?allUserIds:new Set([...allUserIds].filter(userId=>userId===session.user.id||preferences[userId]?.visibility==='household'||(preferences[userId]?.visibility==='specific'&&(preferences[userId]?.sharedWith||[]).includes(session.user.id)))),groups=new Map();
    const add=(record,source)=>{if(!visibleUserIds.has(record.userId)||!['movie','tv'].includes(record.domain))return;const item=resolveCollectionMedia(record.domain,libraries[record.domain]||[],record);if(!item)return;const group=groups.get(record.userId)||{user:{id:record.userId,name:userMap.get(record.userId)?.name||userMap.get(record.userId)?.username||'Deleted user',username:userMap.get(record.userId)?.username||'deleted'},movies:[],television:[]},target=record.domain==='movie'?group.movies:group.television,key=`${record.domain}:${item.id}`;if(!target.some(value=>value.collectionKey===key))target.push({...item,domain:record.domain,collectionKey:key,requestedAt:record.requestedAt||record.addedAt||null,collectionSource:source,requestStatus:record.status||'saved'});groups.set(record.userId,group);};
    for(const record of stored.requests||[])if(!['rejected','canceled','failed','pending_approval'].includes(record.status))add(record,'request');for(const record of stored.interests||[])add(record,'saved');
  ...
  async function overlayForItem(domain,item,configuration=null){const state=configuration||await posterOverlayConfiguration();return resolveOverlayTemplate(item,domain,state.templates,state.assignments);}
  async function televisionFileMetadata(item,template){
    if(!templateUsesVariables(template,fileOverlayVariables))return item.fileMetadata||null;
    const key=collectionMediaId('tv',item.id),cached=posterFileMetadataCache.get(key);let files=cached?.expires>Date.now()?cached.files:null;
    if(!files){const adapter=registry.get('tv');files=typeof adapter.getSeriesFileMetadata==='function'?await adapter.getSeriesFileMetadata(item.id).catch(()=>[]):[];posterFileMetadataCache.set(key,{files,expires:Date.now()+10*60_000});}
  ...
  const publicPlexPosterApplication=value=>({id:value.id,title:value.title,domain:value.domain,templateName:value.templateName,plexLibraryTitle:value.plexLibraryTitle,appliedAt:value.appliedAt,restoredAt:value.restoredAt||null,status:value.restoredAt?'restored':'applied'});
  async function renderedPlexPoster(target,template,session){
    const{item,context}=await overlayRenderContext(target.domain,target.item,template,session),overlay=renderOverlaySvg({poster:Buffer.alloc(0),template,item,context,includePoster:false}),sharp=(await import('sharp')).default;
    const rendered=await sharp(target.poster.body).rotate().resize(600,900,{fit:'cover',position:'centre'}).composite([{input:overlay,top:0,left:0}]).jpeg({quality:92,chromaSubsampling:'4:4:4'}).toBuffer();
    if(!rendered.length||rendered.length>20_000_000)throw new Error('The rendered Plex poster is empty or too large');return rendered;
  }
  async function applyPlexPoster(input,template,session){
    const domain=input.domain==='tv'?'tv':'movie',mediaId=String(input.mediaId||''),libraryKey=String(input.libraryKey||''),ratingKey=String(input.ratingKey||'');if(!/^(?:movie|series)_[A-Za-z0-9_-]+$/.test(mediaId)||!/^\d+$/.test(ratingKey))throw new Error('Choose a valid matched Plex title');if(template.target!=='plex')throw new Error(`${template.name} was not created for Plex artwork`);if(template.domain!=='all'&&template.domain!==domain)throw new Error(`${template.name} is not compatible with this title`);
    const target=await plexPreviewTarget({domain,mediaId,libraryKey,ratingKey}),rendered=await renderedPlexPoster(target,template,session),id=`plex_poster_${randomUUID()}`,backupFile=`${id}.poster`,backupPath=join(plexPosterBackupDir,backupFile);await mkdir(plexPosterBackupDir,{recursive:true});await writeFile(backupPath,target.poster.body,{mode:0o600,flag:'wx'});await plexService.uploadPoster(target.settings.endpoint,target.token,target.plex.ratingKey,rendered,'image/jpeg');const application={id,serverMachineIdentifier:target.settings.server?.machineIdentifier||'',libraryKey:String(target.library.key),plexLibraryTitle:target.library.title,ratingKey:String(target.plex.ratingKey),domain,mediaId,title:target.item.title,templateId:template.id,templateName:template.name,backupFile,backupContentType:target.poster.contentType,backupSha256:createHash('sha256').update(target.poster.body).digest('hex'),renderedSha256:createHash('sha256').update(rendered).digest('hex'),appliedAt:new Date().toISOString(),restoredAt:null};await plexPosterApplicationStore.update(state=>{state.applications=state.applications||[];state.applications.push(application);});await recordAudit(session,{category:'configuration',action:'plex.poster_applied',target:target.item.title,summary:'Applied a reviewed poster overlay to Plex after capturing rollback artwork.',metadata:{applicationId:id,library:target.library.title,template:template.name,ratingKey:target.plex.ratingKey,rollbackCaptured:true}});return application;
  }
  ...
  async function requestAttribution(domain,mediaIds,session){
    const [stored,users]=await Promise.all([requestStore.read(),auth.listUsers()]),userMap=new Map(users.map(user=>[user.id,user])),targets=new Map();for(const value of mediaIds.map(String)){const normalized=collectionMediaId(domain,value);targets.set(normalized,[...(targets.get(normalized)||[]),value]);}const visible=session.user.role==='administrator'?null:new Set([session.user.id]),result={};
    for(const record of stored.requests||[]){const keys=targets.get(collectionMediaId(domain,record.engineId))||[];if(record.domain!==domain||!keys.length||['rejected','canceled','failed','pending_approval'].includes(record.status)||visible&&!visible.has(record.userId))continue;const user=userMap.get(record.userId),attribution={id:record.userId,name:user?.name||user?.username||'Deleted user',username:user?.username||'deleted',requestedAt:record.requestedAt||null,status:record.status,requestId:record.id};for(const target of keys){const key=`${domain}:${target}`,existing=result[key]||[];if(!existing.some(value=>value.id===record.userId))result[key]=[...existing,attribution];}}
    for(const record of stored.interests||[]){const keys=targets.get(collectionMediaId(domain,record.engineId))||[];if(record.domain!==domain||!keys.length||visible&&!visible.has(record.userId))continue;const user=userMap.get(record.userId),attribution={id:record.userId,name:user?.name||user?.username||'Deleted user',username:user?.username||'deleted',requestedAt:record.addedAt||null,status:'saved',requestId:record.id};for(const target of keys){const key=`${domain}:${target}`,existing=result[key]||[];if(!existing.some(value=>value.id===record.userId))result[key]=[...existing,attribution];}}
    return result;
  }
  ...
  async function handleRequest(req,res){
    ...
    try{
      ...
      if(url.pathname.startsWith('/api/')){
        ...
        if(url.pathname==='/api/search-activities'&&req.method==='GET'){if(!administrator(res,session))return;return json(res,200,{items:await reconcileSearchActivities(null)});}
        if(url.pathname==='/api/operations/timeline'&&req.method==='GET'){
          if(!administrator(res,session))return;const domain=url.searchParams.get('domain'),category=url.searchParams.get('category'),status=url.searchParams.get('status'),query=String(url.searchParams.get('query')||'').trim().toLowerCase();let items=await operationsTimeline();if(['movie','tv'].includes(domain))items=items.filter(item=>item.domain===domain);if(category)items=items.filter(item=>item.category===category);if(status)items=items.filter(item=>String(item.status).toLowerCase()===status.toLowerCase());if(query)items=items.filter(item=>`${item.title} ${item.summary} ${item.actor} ${item.source}`.toLowerCase().includes(query));return json(res,200,{items,summary:{total:items.length,categories:Object.fromEntries([...new Set(items.map(item=>item.category))].map(value=>[value,items.filter(item=>item.category===value).length]))}});
        }
        if(url.pathname==='/api/operations/actions'&&req.method==='GET'){
          if(!administrator(res,session))return;const includeDismissed=url.searchParams.get('dismissed')==='true',items=(await operationsActions()).filter(item=>includeDismissed||!item.dismissedAt);return json(res,200,{items,summary:{open:items.filter(item=>!item.dismissedAt).length,critical:items.filter(item=>!item.dismissedAt&&item.severity==='critical').length,warning:items.filter(item=>!item.dismissedAt&&item.severity==='warning').length,dismissed:items.filter(item=>item.dismissedAt).length}});
        }
        const operationActionMatch=url.pathname.match(/^\/api\/operations\/actions\/(.+?)\/(dismiss|restore)$/);
        if(operationActionMatch&&req.method==='POST'){
          if(!administrator(res,session)||!requireCsrf(req,res,session))return;const id=decodeURIComponent(operationActionMatch[1]),action=operationActionMatch[2],known=(await operationsActions()).some(item=>item.id===id);if(!known)return json(res,404,{error:{code:'not_found',message:'This action item is no longer active.'}});await operationsCenterStore.update(current=>{current.version=1;current.dismissed=current.dismissed||{};if(action==='dismiss')current.dismissed[id]=new Date().toISOString();else delete current.dismissed[id];return current;});await recordAudit(session,{category:'operations',action:`action_item.${action}d`,target:id,summary:`${action==='dismiss'?'Dismissed':'Restored'} an Action Center item.`});return json(res,200,{id,dismissed:action==='dismiss'});
        }
        if(url.pathname==='/api/download-decisions'&&req.method==='GET'){
          if(!administrator(res,session))return;const stored=await downloadDecisionStore.read(),domain=url.searchParams.get('domain'),decision=url.searchParams.get('decision'),query=String(url.searchParams.get('query')||'').trim().toLowerCase();let items=stored.decisions||[];if(['movie','tv'].includes(domain))items=items.filter(item=>item.domain===domain);if(['selected','accepted','rejected'].includes(decision))items=items.filter(item=>item.decision===decision);if(query)items=items.filter(item=>`${item.title} ${item.indexer} ${item.quality} ${(item.reasons||[]).join(' ')}`.toLowerCase().includes(query));const attributions={...(await requestAttribution('movie',items.filter(item=>item.domain==='movie').map(item=>item.mediaId),session)),...(await requestAttribution('tv',items.filter(item=>item.domain==='tv').map(item=>item.mediaId),session))};items=items.map(item=>({...item,requesters:attributions[`${item.domain}:${item.mediaId}`]||[]}));return json(res,200,{items:items.slice(0,1000),summary:{total:items.length,selected:items.filter(item=>item.decision==='selected').length,accepted:items.filter(item=>item.decision==='accepted').length,rejected:items.filter(item=>item.decision==='rejected').length}});
        }
        if(url.pathname==='/api/download-decisions'&&req.method==='DELETE'){if(!administrator(res,session)||!requireCsrf(req,res,session))return;await downloadDecisionStore.write({version:1,decisions:[]});await recordAudit(session,{category:'system',action:'download_decisions.cleared',target:'Download Decision Center',summary:'Cleared retained release decision evidence.'});return json(res,200,{cleared:true});}
        const searchActivityMatch=url.pathname.match(/^\/api\/search-activities\/([^/]+)$/);
        if(searchActivityMatch&&req.method==='DELETE'){if(!administrator(res,session)||!requireCsrf(req,res,session))return;let removed=false;await searchActivityStore.update(current=>{const before=(current.activities||[]).length;current.activities=(current.activities||[]).filter(item=>item.id!==searchActivityMatch[1]);removed=current.activities.length<before;return removed;});return removed?json(res,204):json(res,404,{error:{code:'not_found',message:'Search activity was not found'}});}
        ...
        if(url.pathname==='/api/notifications/preferences'&&req.method==='PATCH'){
          if(!requireCsrf(req,res,session))return;const input=await body(req),scope=input.scope==='defaults'?'defaults':'user';if(scope==='defaults'&&!administrator(res,session))return;
          const preferences=sanitizeNotificationPreferences(input.preferences);await notificationStore.update(current=>{current.preferences=current.preferences||{defaults:notificationPreferenceDefaults,users:{}};current.preferences.users=current.preferences.users||{};if(scope==='defaults')current.preferences.defaults=preferences;else current.preferences.users[session.user.id]=preferences;});
          await recordAudit(session,{category:'account',action:'notification_preferences.updated',target:scope,summary:`Updated ${scope==='defaults'?'default':'personal'} notification preferences.`});return json(res,200,await notificationPreferences(session.user.id));
        }
        if(url.pathname==='/api/notifications/channels'&&req.method==='GET'){
          if(!administrator(res,session))return;const stored=await notificationStore.read();return json(res,200,{channels:(stored.channels||[]).map(item=>({...item,credentialConfigured:true,credential:'••••••••'})),deliveries:(stored.deliveries||[]).slice(0,50)});
        }
        if(url.pathname==='/api/notifications/channels'&&req.method==='POST'){
          if(!administrator(res,session)||!requireCsrf(req,res,session))return;const input=await body(req),type=String(input.type||'');if(!['discord','telegram','gotify','pushover'].includes(type))return json(res,400,{error:{code:'invalid_provider',message:'Choose Discord, Telegram, Gotify, or Pushover.'}});const id=/^channel_[A-Za-z0-9_-]+$/.test(String(input.id||''))?String(input.id):`channel_${randomUUID()}`,categories=['request','download','import','system','security'].filter(value=>(input.categories||[]).includes(value)),priority=Math.max(-2,Math.min(2,Number(input.pushoverPriority)||0)),channel={id,type,name:String(input.name||type).trim().slice(0,80)||type,enabled:input.enabled!==false,categories:categories.length?categories:['request','download','import','system','security'],chatId:type==='telegram'?String(input.chatId||'').trim().slice(0,100):'',endpoint:type==='gotify'?String(input.endpoint||'').trim().slice(0,500):'',devices:type==='pushover'?(input.devices||[]).map(value=>String(value).trim()).filter(Boolean).slice(0,20):[],pushoverPriority:priority,retry:priority===2?Math.max(30,Math.min(86400,Number(input.retry)||60)):0,expire:priority===2?Math.max(30,Math.min(86400,Number(input.expire)||3600)):0,ttl:Math.max(0,Math.min(2592000,Number(input.ttl)||0)),sound:/^[A-Za-z0-9_-]{0,40}$/.test(String(input.sound||''))?String(input.sound||''):'',template:sanitizeChannelTemplate(input.template)};if(type==='telegram'&&!channel.chatId)return json(res,400,{error:{code:'chat_id_required',message:'Telegram chat ID is required.'}});if(type==='gotify'&&!/^https?:\/\//i.test(channel.endpoint))return json(res,400,{error:{code:'endpoint_required',message:'Enter a valid Gotify server URL.'}});if(channel.template.json)try{const parsed=JSON.parse(channel.template.json);if(!parsed||Array.isArray(parsed)||typeof parsed!=='object')throw new Error();}catch{return json(res,400,{error:{code:'invalid_notification_json',message:'Custom notification JSON must be a valid object.'}});}const existing=(await notificationStore.read()).channels?.find(item=>item.id===id);if(type==='pushover'){const saved=existing?pushoverCredential(await engineSettings.notificationCredential(id)):{token:'',userKey:'',encryptionKey:''},secret={token:String(input.credential||saved.token).trim(),userKey:String(input.userKey||saved.userKey).trim(),encryptionKey:String(input.encryptionKey||saved.encryptionKey).trim()};if(!secret.token||!secret.userKey)return json(res,400,{error:{code:'pushover_credentials_required',message:'Pushover application token and user key are required.'}});if(secret.encryptionKey&&!/^[0-9a-fA-F]{64}$/.test(secret.encryptionKey))return json(res,400,{error:{code:'invalid_encryption_key',message:'The optional Pushover encryption key must contain exactly 64 hexadecimal characters.'}});await engineSettings.saveNotificationCredential(id,JSON.stringify(secret));}else if(input.credential)await engineSettings.saveNotificationCredential(id,input.credential);else if(!existing)return json(res,400,{error:{code:'credential_required',message:`${type==='discord'?'Webhook URL':'Token'} is required.`}});await notificationStore.update(current=>{current.channels=current.channels||[];const index=current.channels.findIndex(item=>item.id===id);if(index>=0)current.channels[index]=channel;else current.channels.push(channel);});await recordAudit(session,{category:'configuration',action:'notification_channel.saved',target:channel.name,summary:`Saved ${type} notification delivery and message template.`,metadata:{customJson:Boolean(channel.template.json)}});return json(res,200,{channel:{...channel,credentialConfigured:true,credential:'••••••••'}});
        }
        const notificationChannelMatch=url.pathname.match(/^\/api\/notifications\/channels\/(channel_[A-Za-z0-9_-]+)\/(test|retry)$/);
        if(notificationChannelMatch&&req.method==='POST'){
          if(!administrator(res,session)||!requireCsrf(req,res,session))return;const [,channelId,action]=notificationChannelMatch,stored=await notificationStore.read(),channel=(stored.channels||[]).find(item=>item.id===channelId);if(!channel)return json(res,404,{error:{code:'not_found',message:'Notification channel was not found.'}});let event={id:`test:${Date.now()}`,category:'system',severity:'information',title:'VynodeArr test notification',message:'External notification delivery is working.'},attempt=1;if(action==='retry'){const input=await body(req),delivery=(stored.deliveries||[]).find(item=>item.id===input.deliveryId),storedEvent=(stored.events||[]).find(item=>item.id===delivery?.eventId);if(!delivery||!storedEvent)return json(res,404,{error:{code:'delivery_not_found',message:'The delivery event is no longer available.'}});event=storedEvent;attempt=Number(delivery.attempt||1)+1;}try{await sendExternalNotification(channel,event);const delivery=await recordExternalDelivery(channel,event,'delivered','',attempt);return json(res,200,{delivery});}catch(error){const message=error instanceof Error?error.message:String(error),delivery=await recordExternalDelivery(channel,event,'failed',message,attempt);return json(res,502,{error:{code:'delivery_failed',message},delivery});}
        }
        const deleteNotificationChannel=url.pathname.match(/^\/api\/notifications\/channels\/(channel_[A-Za-z0-9_-]+)$/);if(deleteNotificationChannel&&req.method==='DELETE'){
          if(!administrator(res,session)||!requireCsrf(req,res,session))return;const id=deleteNotificationChannel[1];await notificationStore.update(current=>{current.channels=(current.channels||[]).filter(item=>item.id!==id);});await engineSettings.removeNotificationCredential(id);await recordAudit(session,{category:'configuration',action:'notification_channel.removed',target:id,summary:'Removed an external notification channel.'});return json(res,200,{removed:id});
        }
        if(url.pathname==='/api/notifications/test'&&req.method==='POST'){
          if(!requireCsrf(req,res,session))return;const timestamp=new Date().toISOString(),event={id:`test:${session.user.id}:${Date.now()}`,eventGroup:'test',category:'system',severity:'information',type:'imported',title:'Test notification received',message:'Your in-app notification settings are working.',timestamp,href:'#account',requestId:'',actionable:false};await persistNotificationEvents(session.user.id,[event]);return json(res,201,{event});
        }
        ...
        if(url.pathname==='/api/system/validation'&&req.method==='GET'){if(!administrator(res,session))return;const report=await systemValidation();await validationStore.write({version:1,report,updatedAt:report.generatedAt});return json(res,200,report);}
        if(url.pathname==='/api/system/validation/repair'&&req.method==='POST'){
          if(!administrator(res,session)||!requireCsrf(req,res,session))return;const input=await body(req),action=String(input.action||'');
          if(action==='synchronize'){sync.invalidate();await sync.startup();await recordAudit(session,{category:'system',action:'validation.synchronized',target:'System validation',summary:'Re-synchronized both media engines from the Validation center.'});return json(res,200,{repaired:true,action,report:await systemValidation()});}
          if(action==='engine-connections'){if(String(env.VYNODEARR_BUNDLED_ENGINES||'false')!=='true')return json(res,409,{error:{code:'repair_unavailable',message:'Automatic connection repair is available only for installation-managed engines.'}});const repaired=await repairBundledConnections();await sync.startup();await recordAudit(session,{category:'configuration',action:'validation.engine_connections_repaired',target:'Engine connections',summary:'Repaired installation-managed engine connections from the Validation center.'});return json(res,200,{repaired:true,action,result:repaired,report:await systemValidation()});}
        ...
        if(url.pathname==='/api/poster-overlays'&&req.method==='GET'){
          if(!administrator(res,session))return;const configuration=await posterOverlayConfiguration();return json(res,200,{...configuration,variables:posterVariables});
        }
        if(url.pathname==='/api/poster-overlays/plex'&&req.method==='GET'){
          if(!administrator(res,session))return;const settings=await plexSettingsStore.read(),credential=await engineSettings.plexCredential();return json(res,200,{configured:Boolean(credential&&settings.endpoint),endpoint:settings.endpoint||'',server:settings.server||null,libraries:settings.libraries||[],updatedAt:settings.updatedAt||null,artworkWritesEnabled:true});
        }
        if(url.pathname==='/api/poster-overlays/plex'&&req.method==='POST'){
          if(!administrator(res,session)||!requireCsrf(req,res,session))return;const input=await body(req),existingToken=await engineSettings.plexCredential(),token=String(input.token||existingToken||'').trim(),endpoint=sanitizePlexEndpoint(input.endpoint),inspection=await plexService.inspect(endpoint,token);await engineSettings.savePlexCredential(token);await plexSettingsStore.write({version:1,...inspection,updatedAt:new Date().toISOString()});await recordAudit(session,{category:'configuration',action:'plex.connection_saved',target:inspection.server.name,summary:'Validated and saved the encrypted Plex connection for poster artwork review.',metadata:{libraryCount:inspection.libraries.length,artworkWritesEnabled:true}});return json(res,200,{configured:true,...inspection,updatedAt:new Date().toISOString(),artworkWritesEnabled:true});
        }
        if(url.pathname==='/api/poster-overlays/plex'&&req.method==='DELETE'){
          if(!administrator(res,session)||!requireCsrf(req,res,session))return;await engineSettings.removePlexCredential();await plexSettingsStore.write({version:1,endpoint:'',server:null,libraries:[],updatedAt:new Date().toISOString()});await recordAudit(session,{category:'configuration',action:'plex.connection_removed',target:'Plex Media Server',summary:'Removed the saved Plex poster-artwork connection.'});return json(res,200,{configured:false,endpoint:'',server:null,libraries:[],artworkWritesEnabled:true});
        }
        if(url.pathname==='/api/poster-overlays/plex/matches'&&req.method==='POST'){
          if(!administrator(res,session)||!requireCsrf(req,res,session))return;const input=await body(req),settings=await plexSettingsStore.read(),token=await engineSettings.plexCredential();if(!settings.endpoint||!token)return json(res,400,{error:{code:'plex_not_configured',message:'Connect Plex before reviewing library matches.'}});const requested=new Set((input.libraryKeys||[]).map(String)),libraries=(settings.libraries||[]).filter(item=>requested.has(String(item.key)));if(!libraries.length)return json(res,400,{error:{code:'plex_library_required',message:'Choose at least one discovered Plex library.'}});const entries=[];for(const library of libraries){const domain=library.type==='movie'?'movie':'tv',vynode=(await sync.list(domain)).map(item=>({...item,domain})),plexItems=await plexService.libraryItems(settings.endpoint,token,library),matches=typeof plexService.matchLibrary==='function'?plexService.matchLibrary(vynode,plexItems):plexService.match(vynode,plexItems);entries.push(...matches.map(item=>({...item,plexLibrary:{key:library.key,title:library.title,type:library.type}})));}const summary={matched:entries.filter(item=>item.status==='matched').length,unmatched:entries.filter(item=>item.status==='unmatched').length,ambiguous:entries.filter(item=>item.status==='ambiguous').length,total:entries.length};await recordAudit(session,{category:'configuration',action:'plex.match_reviewed',target:settings.server?.name||'Plex Media Server',summary:'Reviewed external-ID matches before Plex poster application.',metadata:{libraryCount:libraries.length,...summary}});return json(res,200,{generatedAt:new Date().toISOString(),summary,entries,artworkWritesEnabled:true});
        }
        if(url.pathname==='/api/poster-overlays/plex/applications'&&req.method==='GET'){
          if(!administrator(res,session))return;const stored=await plexPosterApplicationStore.read();return json(res,200,{applications:(stored.applications||[]).slice().reverse().slice(0,100).map(publicPlexPosterApplication)});
        }
        if(url.pathname==='/api/poster-overlays/plex/apply'&&req.method==='POST'){
          if(!administrator(res,session)||!requireCsrf(req,res,session))return;const input=await body(req);if(String(input.confirmation||'')!=='APPLY TO PLEX')return json(res,400,{error:{code:'confirmation_required',message:'Type APPLY TO PLEX to confirm this one-title poster upload.'}});
          const configuration=await posterOverlayConfiguration(),template=configuration.templates.find(item=>item.id===String(input.templateId||''));if(!template||!template.enabled)return json(res,400,{error:{code:'invalid_template',message:'Choose a compatible poster style.'}});const application=await applyPlexPoster(input,template,session);return json(res,201,{application:publicPlexPosterApplication(application)});
        }
        if(url.pathname==='/api/poster-overlays/plex/apply-batch'&&req.method==='POST'){
          if(!administrator(res,session)||!requireCsrf(req,res,session))return;const input=await body(req,500_000),targets=Array.isArray(input.targets)?input.targets:[],unique=new Set(targets.map(item=>`${item?.libraryKey}:${item?.domain}:${item?.mediaId}:${item?.ratingKey}`));if(!targets.length||targets.length>500||unique.size!==targets.length)return json(res,400,{error:{code:'invalid_plex_targets',message:'Choose between 1 and 500 distinct matched Plex titles.'}});if(String(input.confirmation||'')!==`APPLY ${targets.length} TO PLEX`)return json(res,400,{error:{code:'confirmation_required',message:`Type APPLY ${targets.length} TO PLEX to confirm these poster uploads.`}});const configuration=await posterOverlayConfiguration(),template=configuration.templates.find(item=>item.id===String(input.templateId||''));if(!template||!template.enabled)return json(res,400,{error:{code:'invalid_template',message:'Choose a compatible poster style.'}});const applications=[],failures=[];for(const target of targets){try{applications.push(publicPlexPosterApplication(await applyPlexPoster(target,template,session)));}catch(error){failures.push({mediaId:String(target?.mediaId||''),title:String(target?.title||'Unknown title'),message:redact(error?.message||'Poster application failed')});}}await recordAudit(session,{category:'configuration',action:'plex.poster_batch_completed',target:template.name,summary:`Completed a Plex poster batch with ${applications.length} applied and ${failures.length} failed.`,metadata:{requested:targets.length,applied:applications.length,failed:failures.length}});return json(res,200,{applications,failures,summary:{requested:targets.length,applied:applications.length,failed:failures.length}});
        }
        const plexPosterRestore=url.pathname.match(/^\/api\/poster-overlays\/plex\/applications\/(plex_poster_[A-Za-z0-9-]+)\/restore$/);
        if(plexPosterRestore&&req.method==='POST'){
          if(!administrator(res,session)||!requireCsrf(req,res,session))return;const input=await body(req);if(String(input.confirmation||'')!=='RESTORE PLEX POSTER')return json(res,400,{error:{code:'confirmation_required',message:'Type RESTORE PLEX POSTER to restore the captured artwork.'}});const stored=await plexPosterApplicationStore.read(),application=(stored.applications||[]).find(item=>item.id===plexPosterRestore[1]);if(!application)return json(res,404,{error:{code:'not_found',message:'The Plex poster rollback record was not found.'}});if(application.restoredAt)return json(res,409,{error:{code:'already_restored',message:'This Plex poster was already restored.'}});if(!/^plex_poster_[A-Za-z0-9-]+\.poster$/.test(String(application.backupFile||'')))throw new Error('The Plex rollback record is invalid');const settings=await plexSettingsStore.read(),token=await engineSettings.plexCredential();if(!settings.endpoint||!token||settings.server?.machineIdentifier!==application.serverMachineIdentifier)return json(res,400,{error:{code:'plex_server_changed',message:'Reconnect the original Plex server before restoring this poster.'}});const original=await readFile(join(plexPosterBackupDir,application.backupFile));if(createHash('sha256').update(original).digest('hex')!==application.backupSha256)throw new Error('The captured Plex rollback poster failed integrity validation');await plexService.uploadPoster(settings.endpoint,token,application.ratingKey,original,application.backupContentType);const restoredAt=new Date().toISOString();await plexPosterApplicationStore.update(state=>{const current=(state.applications||[]).find(item=>item.id===application.id);if(current)current.restoredAt=restoredAt;});application.restoredAt=restoredAt;await recordAudit(session,{category:'configuration',action:'plex.poster_restored',target:application.title,summary:'Restored the exact Plex poster captured before overlay application.',metadata:{applicationId:application.id,library:application.plexLibraryTitle,ratingKey:application.ratingKey,integrityVerified:true}});return json(res,200,{application:publicPlexPosterApplication(application)});
        }
        const plexArtworkPreview=url.pathname.match(/^\/api\/poster-overlays\/plex\/(original|preview)\/(movie|tv)\/((?:movie|series)_[A-Za-z0-9_-]+)$/);
        if(plexArtworkPreview&&req.method==='GET'){
          if(!administrator(res,session))return;const[,modeName,domain,mediaId]=plexArtworkPreview,libraryKey=String(url.searchParams.get('libraryKey')||''),ratingKey=String(url.searchParams.get('ratingKey')||'');if(!/^\d+$/.test(ratingKey))return json(res,400,{error:{code:'invalid_plex_target',message:'Choose a valid matched Plex title.'}});const target=await plexPreviewTarget({domain,mediaId,libraryKey,ratingKey});if(modeName==='original'){res.writeHead(200,{'content-type':target.poster.contentType,'cache-control':'private, max-age=300','x-content-type-options':'nosniff'});return res.end(target.poster.body);}const templateId=String(url.searchParams.get('templateId')||''),configuration=await posterOverlayConfiguration(),template=configuration.templates.find(item=>item.id===templateId);if(!template||!template.enabled||template.target!=='plex'||(template.domain!=='all'&&template.domain!==domain))return json(res,400,{error:{code:'invalid_template',message:'Choose a compatible Plex poster style.'}});const rendered=await renderedPlexPoster(target,template,session);res.writeHead(200,{'content-type':'image/jpeg','content-length':rendered.length,'cache-control':'private, max-age=300','x-content-type-options':'nosniff'});return res.end(rendered);
        }
        if(url.pathname==='/api/poster-overlays/templates'&&req.method==='POST'){
          if(!administrator(res,session)||!requireCsrf(req,res,session))return;const template=sanitizeOverlayTemplate(await body(req));await posterOverlayStore.update(current=>{current.templates=current.templates||[];current.templates.push(template);});clearPosterOverlayCache();await recordAudit(session,{category:'configuration',action:'poster_overlay.template_created',target:template.name,summary:'Created a poster overlay template.'});return json(res,201,{template});
        }
        const overlayTemplateMatch=url.pathname.match(/^\/api\/poster-overlays\/templates\/(overlay_[A-Za-z0-9_-]+)$/);
        if(overlayTemplateMatch&&req.method==='PUT'){
          if(!administrator(res,session)||!requireCsrf(req,res,session))return;const current=await posterOverlayStore.read(),existing=(current.templates||[]).find(item=>item.id===overlayTemplateMatch[1]);if(!existing)return json(res,404,{error:{code:'not_found',message:'Poster overlay template was not found.'}});const template=sanitizeOverlayTemplate(await body(req),existing);await posterOverlayStore.update(state=>{state.templates=(state.templates||[]).map(item=>item.id===template.id?template:item);});clearPosterOverlayCache();await recordAudit(session,{category:'configuration',action:'poster_overlay.template_updated',target:template.name,summary:'Updated a poster overlay template.'});return json(res,200,{template});
        }
        ...
        if(overlayAssignmentMatch&&req.method==='PUT'){
          if(!administrator(res,session)||!requireCsrf(req,res,session))return;const state=await posterOverlayStore.read(),existing=(state.assignments||[]).find(item=>item.id===overlayAssignmentMatch[1]),input=await body(req);if(!existing)return json(res,404,{error:{code:'not_found',message:'Poster overlay assignment was not found.'}});const template=(state.templates||[]).find(item=>item.id===input.templateId);if(!template)return json(res,400,{error:{code:'invalid_template',message:'Choose an existing poster overlay template.'}});if(posterTemplateTarget(template)==='plex')return json(res,400,{error:{code:'template_target_mismatch',message:`${template.name} was created for Plex artwork and cannot be assigned to the VynodeArr library.`}});const assignment=sanitizeOverlayAssignment(input,existing);if(template.domain!=='all'&&template.domain!==assignment.scope.domain)return json(res,400,{error:{code:'template_domain_mismatch',message:`${template.name} can only be applied to ${template.domain==='movie'?'Movies':'Television'}.`}});await posterOverlayStore.update(current=>{current.assignments=(current.assignments||[]).map(item=>item.id===assignment.id?assignment:item);});clearPosterOverlayCache();await recordAudit(session,{category:'configuration',action:'poster_overlay.assignment_updated',target:assignment.name,summary:'Updated a poster overlay assignment.'});return json(res,200,{assignment});
        }
        if(overlayAssignmentMatch&&req.method==='DELETE'){
          if(!administrator(res,session)||!requireCsrf(req,res,session))return;let removed=null;await posterOverlayStore.update(state=>{removed=(state.assignments||[]).find(item=>item.id===overlayAssignmentMatch[1])||null;state.assignments=(state.assignments||[]).filter(item=>item.id!==overlayAssignmentMatch[1]);});if(!removed)return json(res,404,{error:{code:'not_found',message:'Poster overlay assignment was not found.'}});clearPosterOverlayCache();await recordAudit(session,{category:'configuration',action:'poster_overlay.assignment_deleted',target:removed.name,summary:'Deleted a poster overlay assignment.'});return json(res,200,{deleted:true});
        }
        if(url.pathname==='/api/collections'&&req.method==='GET'){
          ...
          return json(res,200,{items:collections,userCollections:await userRequestCollections(session),currentUserId:session.user.id});
        }
        if(url.pathname==='/api/user-collections/sharing'&&req.method==='GET'){
          const stored=await requestStore.read(),users=await auth.listUsers(),target=String(url.searchParams.get('userId')||session.user.id);if(target!==session.user.id&&session.user.role!=='administrator')return json(res,403,{error:{code:'forbidden',message:'You cannot manage another user collection.'}});return json(res,200,{preference:stored.collectionPreferences?.[target]||{visibility:'private',sharedWith:[]},users:users.filter(user=>user.id!==target&&!user.disabled).map(({id,name,username})=>({id,name,username}))});
        }
        if(url.pathname==='/api/user-collections/sharing'&&req.method==='PUT'){
          if(!requireCsrf(req,res,session))return;const input=await body(req),target=String(input.userId||session.user.id);if(target!==session.user.id&&session.user.role!=='administrator')return json(res,403,{error:{code:'forbidden',message:'You cannot manage another user collection.'}});const visibility=['private','household','specific'].includes(input.visibility)?input.visibility:'private',users=await auth.listUsers(),allowed=new Set(users.map(user=>user.id)),sharedWith=[...new Set((input.sharedWith||[]).map(String).filter(id=>id!==target&&allowed.has(id)))];await requestStore.update(current=>{current.collectionPreferences=current.collectionPreferences||{};current.collectionPreferences[target]={visibility,sharedWith:visibility==='specific'?sharedWith:[]};});await recordAudit(session,{category:'collection',action:'user_collection.sharing_updated',target,summary:`Changed user collection visibility to ${visibility}.`,metadata:{visibility,sharedWithCount:sharedWith.length}});return json(res,200,{preference:{visibility,sharedWith:visibility==='specific'?sharedWith:[]}});
        }
        if(url.pathname==='/api/request-attribution'&&req.method==='GET'){
          const domain=String(url.searchParams.get('domain')||''),mediaIds=String(url.searchParams.get('mediaIds')||'').split(',').map(value=>value.trim()).filter(Boolean).slice(0,1000);if(!['movie','tv'].includes(domain))return json(res,400,{error:{code:'invalid_domain',message:'Choose Movies or Television.'}});return json(res,200,{items:await requestAttribution(domain,mediaIds,session)});
        }
        if(url.pathname==='/api/user-collections/timeline'&&req.method==='GET'){
          const target=String(url.searchParams.get('userId')||session.user.id);if(target!==session.user.id&&session.user.role!=='administrator')return json(res,403,{error:{code:'forbidden',message:'You cannot view another user timeline.'}});const [stored,activities,decisions,history]=await Promise.all([requestStore.read(),searchActivityStore.read(),downloadDecisionStore.read(),dashboardHistory().catch(()=>[])]),requests=(stored.requests||[]).filter(item=>item.userId===target),keys=new Map(requests.filter(item=>item.engineId!=null).map(item=>[`${item.domain}:${item.engineId}`,item]));const events=[];for(const item of requests){events.push({id:`${item.id}:requested`,type:'request',domain:item.domain,mediaId:item.engineId,title:item.title,status:item.status,timestamp:item.requestedAt||item.createdAt||item.updatedAt,detail:item.message||'Media requested'});if(item.approvedAt)events.push({id:`${item.id}:approved`,type:'approval',domain:item.domain,mediaId:item.engineId,title:item.title,status:'approved',timestamp:item.approvedAt,detail:'Request approved'});if(item.cancelledAt)events.push({id:`${item.id}:canceled`,type:'cancel',domain:item.domain,mediaId:item.engineId,title:item.title,status:'canceled',timestamp:item.cancelledAt,detail:'Request canceled'});}for(const item of activities.activities||[]){const request=keys.get(`${item.domain}:${item.mediaId}`);if(request)events.push({id:item.id,type:'search',domain:item.domain,mediaId:item.mediaId,title:item.title||request.title,status:item.status,timestamp:item.updatedAt||item.createdAt,detail:item.message||'Automatic search activity'});}for(const item of decisions.decisions||[]){if(item.userId===target)events.push({id:item.id,type:'decision',domain:item.domain,mediaId:item.mediaId,title:item.title,status:item.decision,timestamp:item.observedAt,detail:(item.reasons||[])[0]||'Download decision recorded'});}for(const item of history){const request=keys.get(`${item.domain}:${item.mediaId}`);if(request)events.push({id:`history:${item.id}`,type:'history',domain:item.domain,mediaId:item.mediaId,title:item.title||request.title,status:item.eventType||'history',timestamp:item.timestamp,detail:item.details||item.context||'Library activity'});}return json(res,200,{items:events.filter(item=>item.timestamp).sort((a,b)=>String(b.timestamp).localeCompare(String(a.timestamp))).slice(0,1000)});
        }
        if(url.pathname==='/api/user-collections/export'&&req.method==='GET'){
          const target=String(url.searchParams.get('userId')||session.user.id),format=String(url.searchParams.get('format')||'json');if(target!==session.user.id&&session.user.role!=='administrator')return json(res,403,{error:{code:'forbidden',message:'You cannot export another user collection.'}});const collection=(await userRequestCollections({...session,user:{...session.user,role:'administrator'}})).find(item=>item.user.id===target);if(!collection)return json(res,404,{error:{code:'not_found',message:'Collection not found.'}});const records=[...collection.movies,...collection.television].map(({domain,id,title,year,tmdbId,tvdbId,collectionSource})=>({domain,id,title,year:year||'',tmdbId:tmdbId||'',tvdbId:tvdbId||'',source:collectionSource}));if(format==='csv'){const escape=value=>`"${String(value??'').replace(/"/g,'""')}"`,csv=['domain,id,title,year,tmdbId,tvdbId,source',...records.map(row=>[row.domain,row.id,row.title,row.year,row.tmdbId,row.tvdbId,row.source].map(escape).join(','))].join('\r\n');res.writeHead(200,{'content-type':'text/csv; charset=utf-8','content-disposition':`attachment; filename="vynodearr-${collection.user.username}-collection.csv"`,'cache-control':'no-store'});return res.end(csv);}return json(res,200,{version:1,exportedAt:new Date().toISOString(),user:collection.user,items:records},{'content-disposition':`attachment; filename="vynodearr-${collection.user.username}-collection.json"`});
        }
        if(url.pathname==='/api/user-collections/import'&&req.method==='POST'){
          if(!requireCsrf(req,res,session))return;const input=await body(req,5_000_000),target=String(input.userId||session.user.id);if(target!==session.user.id&&session.user.role!=='administrator')return json(res,403,{error:{code:'forbidden',message:'You cannot import into another user collection.'}});const libraries={movie:await sync.list('movie'),tv:await sync.list('tv')},items=Array.isArray(input.items)?input.items.slice(0,5000):[],added=[];for(const candidate of items){const domain=String(candidate.domain||''),library=libraries[domain]||[],match=library.find(item=>String(item.id)===String(candidate.id)||candidate.tmdbId&&String(item.tmdbId)===String(candidate.tmdbId)||candidate.tvdbId&&String(item.tvdbId)===String(candidate.tvdbId));if(match)added.push({id:`interest_${randomUUID()}`,userId:target,domain,engineId:match.id,tmdbId:match.tmdbId||null,tvdbId:match.tvdbId||null,title:match.title,addedAt:new Date().toISOString()});}await requestStore.update(current=>{current.interests=current.interests||[];const existing=new Set(current.interests.map(item=>`${item.userId}:${item.domain}:${item.engineId}`));for(const item of added)if(!existing.has(`${item.userId}:${item.domain}:${item.engineId}`)){current.interests.push(item);existing.add(`${item.userId}:${item.domain}:${item.engineId}`);}});await recordAudit(session,{category:'collection',action:'user_collection.imported',target,summary:`Imported ${added.length} matching collection titles.`,metadata:{matched:added.length,submitted:items.length}});return json(res,200,{matched:added.length,submitted:items.length});
        }
        if(url.pathname==='/api/user-collections/bulk'&&req.method==='POST'){
          if(!requireCsrf(req,res,session))return;const input=await body(req),target=String(input.userId||session.user.id),action=String(input.action||''),items=(input.items||[]).filter(item=>['movie','tv'].includes(item.domain)&&String(item.id||'')).slice(0,500);if(target!==session.user.id&&session.user.role!=='administrator')return json(res,403,{error:{code:'forbidden',message:'You cannot change another user collection.'}});if(!['remove','search','monitor','unmonitor','profile'].includes(action))return json(res,400,{error:{code:'invalid_action',message:'Choose a supported bulk action.'}});if(action!=='remove'&&session.user.role!=='administrator')return json(res,403,{error:{code:'administrator_required',message:'Administrator access is required for library management actions.'}});let completed=0,failed=0;if(action==='remove'){await requestStore.update(current=>{const keys=new Set(items.map(item=>`${item.domain}:${item.id}`)),before=(current.interests||[]).length;current.interests=(current.interests||[]).filter(item=>item.userId!==target||!keys.has(`${item.domain}:${item.engineId}`));completed=before-current.interests.length;});}else for(const item of items){try{const client=registry.get(item.domain).client,id=Number(String(item.id).replace(/^(movie|series)_/,''));if(!Number.isFinite(id))throw new Error('Engine ID unavailable');if(action==='search')await client.post('command',item.domain==='movie'?{name:'MoviesSearch',movieIds:[id]}:{name:'SeriesSearch',seriesId:id});else{const path=`${item.domain==='movie'?'movie':'series'}/${id}`,record=await client.get(path);if(action==='profile')record.qualityProfileId=Number(input.qualityProfileId);else record.monitored=action==='monitor';await client.put(path,record);}completed++;}catch{failed++;}}await recordAudit(session,{category:'collection',action:`user_collection.bulk_${action}`,target,summary:`Applied ${action} to ${completed} collection title${completed===1?'':'s'}.`,metadata:{completed,failed}});return json(res,200,{completed,failed});
        }
        if(url.pathname==='/api/user-collections/contains'&&req.method==='GET'){
          const domain=String(url.searchParams.get('domain')||''),engineId=String(url.searchParams.get('mediaId')||'').trim();if(!['movie','tv'].includes(domain)||!engineId)return json(res,400,{error:{code:'invalid_media',message:'Choose a valid library title.'}});const collections=await userRequestCollections(session),mine=collections.find(item=>item.user.id===session.user.id),items=domain==='movie'?mine?.movies:mine?.television,member=items?.find(item=>collectionMediaId(domain,item.id)===collectionMediaId(domain,engineId));return json(res,200,{included:Boolean(member),source:member?.collectionSource||null,canRemove:member?.collectionSource==='saved'});
        }
        if(url.pathname==='/api/user-collections/attribution'&&req.method==='GET'){
          const domain=String(url.searchParams.get('domain')||''),engineId=String(url.searchParams.get('mediaId')||'').trim();if(!['movie','tv'].includes(domain)||!engineId)return json(res,400,{error:{code:'invalid_media',message:'Choose a valid library title.'}});const collections=await userRequestCollections(session),users=collections.flatMap(collection=>{const items=domain==='movie'?collection.movies:collection.television,member=items.find(item=>collectionMediaId(domain,item.id)===collectionMediaId(domain,engineId));return member?[{...collection.user,source:member.collectionSource,requestedAt:member.requestedAt||null}]:[];});return json(res,200,{users});
        }
        if(url.pathname==='/api/user-collections/items'&&req.method==='POST'){
          if(!requireCsrf(req,res,session))return;const input=await body(req),domain=String(input.domain||''),engineId=String(input.mediaId||'').trim();if(!['movie','tv'].includes(domain)||!engineId)return json(res,400,{error:{code:'invalid_media',message:'Choose a valid library title.'}});const library=await sync.list(domain),media=resolveCollectionMedia(domain,library,{...input,engineId});if(!media)return json(res,404,{error:{code:'not_found',message:'This title could not be matched to the current library. Refresh the library and try again.'}});const canonicalId=String(media.id),identity={engineId:canonicalId,tmdbId:media.tmdbId||input.tmdbId||null,tvdbId:media.tvdbId||input.tvdbId||null};let interest;await requestStore.update(current=>{current.interests=current.interests||[];interest=current.interests.find(item=>item.userId===session.user.id&&item.domain===domain&&sameCollectionMedia(domain,item,identity));if(!interest){interest={id:`interest_${randomUUID()}`,userId:session.user.id,domain,...identity,title:media.title,year:media.year||null,addedAt:new Date().toISOString()};current.interests.push(interest);}});await recordAudit(session,{category:'collection',action:'user_collection.item_added',target:media.title,domain,summary:`Added ${media.title} to the user's collection.`,metadata:{engineId:canonicalId,tmdbId:identity.tmdbId,tvdbId:identity.tvdbId,interestId:interest.id}});return json(res,201,{item:interest});
        }
        const userCollectionItemMatch=url.pathname.match(/^\/api\/user-collections\/items\/(movie|tv)\/([A-Za-z0-9_-]+)$/);if(userCollectionItemMatch&&req.method==='DELETE'){
          if(!requireCsrf(req,res,session))return;const [,domain,engineId]=userCollectionItemMatch;let removed=null;await requestStore.update(current=>{const interests=current.interests||[];removed=interests.find(item=>item.userId===session.user.id&&item.domain===domain&&collectionMediaId(domain,item.engineId)===collectionMediaId(domain,engineId))||null;current.interests=interests.filter(item=>item!==removed);});if(removed)await recordAudit(session,{category:'collection',action:'user_collection.item_removed',target:removed.title,domain,summary:`Removed ${removed.title} from the user's collection.`,metadata:{engineId,interestId:removed.id}});return json(res,200,{removed:Boolean(removed)});
        }
        ...
        if(automaticSearchMatch&&req.method==='POST'){
          ...
          await recordDownloadDecisions(session.user.id,domain,query,candidates,{source:'automatic'});
          ...
          await recordDownloadDecisions(session.user.id,domain,query,candidates,{source:'automatic',selected:releaseIdentity(selected)});
        ...
        if(managementMatch){
          ...
          if(managementMatch[2]==='releases'&&method==='GET'){
            ...
            await recordDownloadDecisions(session.user.id,domain,query,result,{source:'interactive'});
          }
          else{
            ...
            if(method==='POST'&&managementMatch[2]==='releases'){
              const domain=managementMatch[1],movieId=Number(payload.mappedMovieId||payload.movieId)||null,mappedEpisodes=Array.isArray(payload.mappedEpisodeInfo)?payload.mappedEpisodeInfo:[],episodeIds=[payload.episodeId,payload.mappedEpisodeId,...mappedEpisodes.map(item=>item.id)].map(Number).filter(Number.isFinite),seriesId=Number(payload.seriesId||payload.series?.id||mappedEpisodes[0]?.seriesId)||null,libraryMedia=movieId?await management.execute('movie','library','GET',{id:String(movieId)}).catch(()=>null):seriesId?await management.execute('tv','library','GET',{id:String(seriesId)}).catch(()=>null):null,mediaTitle=libraryMedia?.title||payload.movie?.title||payload.series?.title||input.movie?.title||input.series?.title||payload.title||'Interactive release',quality=payload.quality?.quality?.name||payload.quality?.name||'Unknown';
              const activity=await createSearchActivity(session.user.id,domain==='movie'?{name:'MoviesSearch',movieIds:movieId?[movieId]:[]}:{name:'EpisodeSearch',episodeIds},{},{domain,source:'interactive',scope:domain==='movie'?'movie':episodeIds.length>1?'episode':'episode',movieId,seriesId,episodeIds,status:'grabbed',title:mediaTitle,message:'A manually selected release was grabbed and sent to the download client.',selection:{title:payload.title||mediaTitle,quality,size:Number(payload.size||0)}});
              await recordDownloadDecisions(session.user.id,domain,{movieId,episodeId:episodeIds[0],seriesId},[payload],{source:'interactive',selected:releaseIdentity(payload)});
              await persistNotificationEvents(session.user.id,[{id:`interactive-grab:${activity.id}`,eventGroup:'interactive-grab',category:'download',severity:'success',type:'grabbed',title:`${mediaTitle} was grabbed`,message:`The selected ${quality} release was sent to the download client.`,timestamp:activity.createdAt,href:'#queue',requestId:'',actionable:false}]);
              await recordAudit(session,{category:'media',action:'interactive_search.grabbed',target:mediaTitle,domain,summary:`Manually selected and grabbed a ${domain==='movie'?'movie':'television'} release.`,metadata:{activityId:activity.id,mediaId:movieId||seriesId||episodeIds[0]||null,quality}});
            }
