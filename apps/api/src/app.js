...
import { overlayRevision,posterVariableValues,posterVariables,renderOverlaySvg,resolveOverlayTemplate,sanitizeOverlayAssignment,sanitizeOverlayLayer,sanitizeOverlayTemplate } from '../../../packages/platform/src/poster-overlay-service.js';
...
const importJobs=new Map(),searchJobs=new Map(),namingAuditJobs=new Map(),completedQueueRefreshes=new Map(),completedQueueCleanups=new Map(),completedUpgradeRenames=new Map(),completedLibraryImports=new Map(),libraryReconciliations=new Map(),libraryEventClients=new Set(),interactiveReleaseCache=new Map(),renamePlans=new Map(),applicationBackupDownloads=new Map();
  ...
  async function decoratePosterArtwork(domain,items,session){const state=await posterOverlayConfiguration(),attribution=session?await requestAttribution(domain,items.map(item=>item.id),session):{};return items.map(item=>{const template=resolveOverlayTemplate(item,domain,state.templates,state.assignments),overlayValues=posterVariableValues(item,{requesters:attribution[`${domain}:${item.id}`]||[]}),artwork={...(item.artwork||{}),overlayValues};return template?{...item,artwork:{...artwork,originalUrl:item.artwork?.url,overlayTemplateId:template.id,overlayTemplate:template}}:{...item,artwork};});}
  function clearPosterOverlayCache(){posterOverlayCache.clear();}
  async function requestAttribution(domain,mediaIds,session){
    const [stored,users]=await Promise.all([requestStore.read(),auth.listUsers()]),userMap=new Map(users.map(user=>[user.id,user])),ids=new Set(mediaIds.map(String)),visible=session.user.role==='administrator'?null:new Set([session.user.id]),result={};
    for(const record of stored.requests||[]){if(record.domain!==domain||!ids.has(String(record.engineId))||['rejected','canceled','failed','pending_approval'].includes(record.status)||visible&&!visible.has(record.userId))continue;const key=`${domain}:${record.engineId}`,user=userMap.get(record.userId);result[key]=[...(result[key]||[]),{id:record.userId,name:user?.name||user?.username||'Deleted user',username:user?.username||'deleted',requestedAt:record.requestedAt||null,status:record.status,requestId:record.id}];}
    for(const record of stored.interests||[]){if(record.domain!==domain||!ids.has(String(record.engineId))||visible&&!visible.has(record.userId))continue;const key=`${domain}:${record.engineId}`,user=userMap.get(record.userId),existing=result[key]||[];if(!existing.some(value=>value.id===record.userId))result[key]=[...existing,{id:record.userId,name:user?.name||user?.username||'Deleted user',username:user?.username||'deleted',requestedAt:record.addedAt||null,status:'saved',requestId:record.id}];}
    return result;
  }
    ...
    try{
      ...
      if(url.pathname.startsWith('/api/')){
        ...
        if(artworkMatch){
          ...
          if(artworkMatch[3]==='poster'&&url.searchParams.has('variant')){try{const domain=artworkMatch[1],library=await sync.list(domain),item=library.find(entry=>entry.id===artworkMatch[2]),template=item?await overlayForItem(domain,item):null;if(item&&template){const revision=overlayRevision(template,item),renderKey=`${domain}:${item.id}:${revision}`;let rendered=posterOverlayCache.get(renderKey);if(!rendered){rendered=renderOverlaySvg({poster:value.body,contentType:value.contentType,template,item});if(posterOverlayCache.size>=500)posterOverlayCache.delete(posterOverlayCache.keys().next().value);posterOverlayCache.set(renderKey,rendered);}res.writeHead(200,{'content-type':'image/svg+xml; charset=utf-8','cache-control':'private, max-age=86400','content-security-policy':"default-src 'none'; img-src data:",'x-content-type-options':'nosniff'});return res.end(rendered);}}catch{} }
        ...
        if(overlayRenderMatch){
          const domain=overlayRenderMatch[1];if(!permitted(res,session,domain==='movie'?'movies':'tv'))return;const library=await sync.list(domain),item=library.find(value=>value.id===overlayRenderMatch[2]);if(!item)return json(res,404,{error:{code:'not_found',message:'Library item was not found.'}});const original=await registry.get(domain).getArtwork(item.id,'poster');if(!original){res.writeHead(204,{'cache-control':'private, max-age=300'});return res.end();}try{const template=await overlayForItem(domain,item);if(!template){res.writeHead(200,{'content-type':original.contentType,'cache-control':'private, max-age=86400','x-content-type-options':'nosniff'});return res.end(original.body);}const revision=overlayRevision(template,item),key=`${domain}:${item.id}:${revision}`;let rendered=posterOverlayCache.get(key);if(!rendered){rendered=renderOverlaySvg({poster:original.body,contentType:original.contentType,template,item});if(posterOverlayCache.size>=500)posterOverlayCache.delete(posterOverlayCache.keys().next().value);posterOverlayCache.set(key,rendered);}res.writeHead(200,{'content-type':'image/svg+xml; charset=utf-8','cache-control':'private, max-age=86400','content-security-policy':"default-src 'none'; img-src data:",'x-content-type-options':'nosniff'});return res.end(rendered);}catch{res.writeHead(200,{'content-type':original.contentType,'cache-control':'private, max-age=300','x-content-type-options':'nosniff'});return res.end(original.body);}
        }
