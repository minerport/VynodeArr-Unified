    ...
    try {
      ...
      if (url.pathname.startsWith("/api/")) {
        ...
        if (url.pathname === "/api/music/import/settings") {
          ...
          if (req.method === "PUT") { if (!requireCsrf(req, res, session)) return; const input=await body(req),current=await musicImports.settings(),next={...current,...input},changesDownload=Object.hasOwn(input,"downloadPath"),changesLibrary=Object.hasOwn(input,"libraryRoot");if(normalizeMediaPath(next.downloadPath)===normalizeMediaPath(next.libraryRoot))throw new Error("Choose separate Music download and library folders");const checked=await decorateMusicFolders(next);if(changesDownload&&!checked.downloadAccessible)throw new Error("The selected Music download folder must exist and be readable and writable inside VynodeArr");if(changesLibrary&&!checked.libraryAccessible)throw new Error("The selected Music library folder must exist and be readable and writable inside VynodeArr");return json(res, 200, await decorateMusicFolders(await musicImports.settings(next))); }
        }
