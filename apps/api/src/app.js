    ...
    try{
      ...
      if(url.pathname.startsWith('/api/')){
        ...
        if(url.pathname==='/api/download-decisions'&&req.method==='GET'){
          if(!administrator(res,session))return;const stored=await downloadDecisionStore.read(),domain=url.searchParams.get('domain'),decision=url.searchParams.get('decision'),query=String(url.searchParams.get('query')||'').trim().toLowerCase();let items=stored.decisions||[];if(['movie','tv'].includes(domain))items=items.filter(item=>item.domain===domain);if(['selected','accepted','rejected'].includes(decision))items=items.filter(item=>item.decision===decision);if(query)items=items.filter(item=>`${item.title} ${item.indexer} ${item.quality} ${(item.reasons||[]).join(' ')}`.toLowerCase().includes(query));return json(res,200,{items:items.slice(0,1000),summary:{total:items.length,selected:items.filter(item=>item.decision==='selected').length,accepted:items.filter(item=>item.decision==='accepted').length,rejected:items.filter(item=>item.decision==='rejected').length}});
        }
