    ...
    try{
      ...
      if(url.pathname.startsWith('/api/')){
        ...
        if(url.pathname==='/api/collections'&&req.method==='GET'){
          ...
          return json(res,200,{items:collections,userCollections:await userRequestCollections(session)});
        }
        ...
        if(url.pathname==='/api/user-collections/items'&&req.method==='POST'){
          if(!requireCsrf(req,res,session))return;const input=await body(req),domain=String(input.domain||''),engineId=String(input.mediaId||'').trim();if(!['movie','tv'].includes(domain)||!engineId)return json(res,400,{error:{code:'invalid_media',message:'Choose a valid library title.'}});const library=await sync.list(domain),media=library.find(item=>String(item.id)===engineId);if(!media)return json(res,404,{error:{code:'not_found',message:'This title is no longer in the library.'}});let interest;await requestStore.update(current=>{current.interests=current.interests||[];interest=current.interests.find(item=>item.userId===session.user.id&&item.domain===domain&&String(item.engineId)===engineId);if(!interest){interest={id:`interest_${randomUUID()}`,userId:session.user.id,domain,engineId,tmdbId:media.tmdbId||null,tvdbId:media.tvdbId||null,title:media.title,addedAt:new Date().toISOString()};current.interests.push(interest);}});await recordAudit(session,{category:'collection',action:'user_collection.item_added',target:media.title,domain,summary:`Added ${media.title} to the user's collection.`,metadata:{engineId,interestId:interest.id}});return json(res,201,{item:interest});
        }
        const userCollectionItemMatch=url.pathname.match(/^\/api\/user-collections\/items\/(movie|tv)\/([A-Za-z0-9_-]+)$/);if(userCollectionItemMatch&&req.method==='DELETE'){
          if(!requireCsrf(req,res,session))return;const [,domain,engineId]=userCollectionItemMatch;let removed=null;await requestStore.update(current=>{const interests=current.interests||[];removed=interests.find(item=>item.userId===session.user.id&&item.domain===domain&&String(item.engineId)===engineId)||null;current.interests=interests.filter(item=>item!==removed);});if(removed)await recordAudit(session,{category:'collection',action:'user_collection.item_removed',target:removed.title,domain,summary:`Removed ${removed.title} from the user's collection.`,metadata:{engineId,interestId:removed.id}});return json(res,200,{removed:Boolean(removed)});
        }
