...
const importJobs=new Map(),searchJobs=new Map(),namingAuditJobs=new Map(),completedQueueRefreshes=new Map(),completedQueueCleanups=new Map(),completedUpgradeRenames=new Map(),completedLibraryImports=new Map(),libraryReconciliations=new Map(),libraryEventClients=new Set(),interactiveReleaseCache=new Map(),renamePlans=new Map(),applicationBackupDownloads=new Map();
  ...
  const templateUsesVariables=(template,set)=>template?.layers?.some(layer=>set.has(layer.variable)||(layer.conditions?.rules||[]).some(rule=>set.has(rule.variable)));
  ...
  async function televisionEpisodeMetadata(item,template){if(!templateUsesVariables(template,episodeOverlayVariables))return{};const key=collectionMediaId('tv',item.id),cached=posterEpisodeMetadataCache.get(key);if(cached?.expires>Date.now())return cached.value;const adapter=registry.get('tv'),value=typeof adapter.getSeriesOverlayMetadata==='function'?await adapter.getSeriesOverlayMetadata(item.id).catch(()=>({})):{};posterEpisodeMetadataCache.set(key,{value,expires:Date.now()+10*60_000});return value;}
