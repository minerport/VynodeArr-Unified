...
export function RequestManagementView({options}:{options:MyRequestsMountOptions}){
  const [items,setItems]=useState<UserRequest[]>([]),[notifications,setNotifications]=useState<NotificationItem[]>([]),[loading,setLoading]=useState(true),[error,setError]=useState(''),[query,setQuery]=useState(''),[userId,setUserId]=useState(''),[status,setStatus]=useState<RequestStatus|''>(''),[domain,setDomain]=useState<''|'movie'|'tv'>(''),[busy,setBusy]=useState(''),[rejecting,setRejecting]=useState<UserRequest|null>(null);
  const load=useCallback(async(quiet=false)=>{if(!quiet)setLoading(true);try{const[value,notificationValue]=await Promise.all([options.request<{items:UserRequest[]}>('/api/requests'),options.request<{items:NotificationItem[]}>('/api/notifications')]),reviewed=await options.request<{reviewed:string[]}>('/api/notifications/review-requests',{method:'POST',body:'{}'}),reviewedIds=new Set(reviewed.reviewed);setItems(value.items||[]);setNotifications((notificationValue.items||[]).filter(item=>Boolean(item.requestId)).map(item=>reviewedIds.has(item.id)?{...item,read:true}:item));if(reviewedIds.size)window.dispatchEvent(new Event('vynodearr:notifications-changed'));setError('');}catch(reason){setError(message(reason));}finally{setLoading(false);}},[options]);
  ...
  const notificationFor=(requestId:string)=>notifications.find(item=>item.requestId===requestId),unreadNotifications=notifications.filter(item=>!item.read);
  const markRead=async(ids:string[])=>{if(!ids.length)return;try{await options.request('/api/notifications/read',{method:'POST',body:JSON.stringify({ids})});setNotifications(current=>current.map(item=>ids.includes(item.id)?{...item,read:true}:item));window.dispatchEvent(new Event('vynodearr:notifications-changed'));}catch(reason){options.notify(message(reason),'error');}};
  return <div className="react-request-management">
    <div className="hero"><div><span className="eyebrow">DISCOVER ADMINISTRATION</span><h1>User Requests</h1><p className="lede">Review request history by user and decide which approval-required titles are added.</p></div><div className="request-header-actions"><button className="secondary" disabled={!unreadNotifications.length} onClick={()=>void markRead(unreadNotifications.map(item=>item.id))}>Mark all read</button><button className="secondary" onClick={()=>void load()}>Refresh</button></div></div>
    ...
    {loading?<div className="panel skeleton">Loading user requests…</div>:error?<div className="panel error-state"><h2>Requests unavailable</h2><p>{error}</p></div>:!visible.length?<div className="panel empty"><h2>No matching requests</h2><p>Change the filters or wait for a user to submit a request.</p></div>:<div className="admin-request-list">{visible.map(record=>{const notification=notificationFor(record.id);return <article className={`admin-request-card panel${notification&&!notification.read?' unread':''}`} key={record.id}>
