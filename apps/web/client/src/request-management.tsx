...
export function RequestManagementView({options}:{options:MyRequestsMountOptions}){
  ...
  const load=useCallback(async(quiet=false)=>{if(!quiet)setLoading(true);try{const[value,notificationValue]=await Promise.all([options.request<{items:UserRequest[]}>('/api/requests'),options.request<{items:NotificationItem[]}>('/api/notifications')]);setItems(value.items||[]);setNotifications((notificationValue.items||[]).filter(item=>Boolean(item.requestId)));setError('');}catch(reason){setError(message(reason));}finally{setLoading(false);}},[options]);
