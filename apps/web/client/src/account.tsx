...
function RequestLimitsField({limits}:{limits?:AccountUser['requestLimits']}){
  return <fieldset className="user-access-fields request-limit-fields"><legend>Discover request limits</legend><p>Leave an allowance blank for unlimited. Disable limits to remove the policy without affecting existing requests.</p><label className="user-access-option"><input type="checkbox" name="requestLimitsEnabled" defaultChecked={limits?.enabled}/><span><strong>Enable request limits</strong><small>Enforce the allowances below for this user.</small></span></label><div className="request-limit-grid"><label>Period<select name="requestLimitPeriod" defaultValue={limits?.period||'weekly'}><option value="daily">Daily</option><option value="weekly">Weekly</option><option value="monthly">Monthly</option></select></label><label>Movies allowed<input name="requestLimitMovie" type="number" min="1" max="10000" defaultValue={limits?.movie??''} placeholder="Unlimited"/></label><label>TV series allowed<input name="requestLimitTv" type="number" min="1" max="10000" defaultValue={limits?.tv??''} placeholder="Unlimited"/></label><label>Maximum pending<input name="requestLimitPending" type="number" min="1" max="10000" defaultValue={limits?.maxPending??''} placeholder="Unlimited"/></label></div></fieldset>;
}
...
function AccountForm({options}:{options:AccountMountOptions}){
  ...
  if(error)return <RouteError title="Account unavailable" message={error}/>;
  if(!user)return <RouteLoading>Loading account…</RouteLoading>;
...
function Sessions({options}:{options:AccountMountOptions}){
  ...
  if(error)return <RouteError title="Sessions unavailable" message={error} onRetry={()=>void load()}/>;
  return <><div className="account-section-actions"><span>{items.length} signed-in session{items.length===1?'':'s'}</span><button className="secondary" disabled={loading} onClick={()=>void revokeOthers()}>Sign out all other sessions</button></div><div className={`panel session-list${loading?' is-loading':''}`}>{items.map(item=><article key={item.id}><div><h2>{item.browser} on {item.os} {item.current?<span className="badge green">Current</span>:null}</h2><p>{item.ipMasked} · Last active {formatDateTime(item.lastActivity)} · Created {formatDateTime(item.createdAt)}</p></div><button className="text-button" onClick={()=>void revoke(item.id)}>{item.current?'Sign out this session':'Revoke'}</button></article>)}</div></>;
}
...
function Users({options}:{options:AccountMountOptions}){
  ...
  const requestLimits=(form:FormData)=>({enabled:form.has('requestLimitsEnabled'),period:String(form.get('requestLimitPeriod')||'weekly'),movie:form.get('requestLimitMovie')||null,tv:form.get('requestLimitTv')||null,maxPending:form.get('requestLimitPending')||null});
  async function create(event:FormEvent<HTMLFormElement>){event.preventDefault();const form=new FormData(event.currentTarget),payload={...Object.fromEntries(form.entries()),permissions:selectedPermissions(form),requestApprovalRequired:form.has('requestApprovalRequired'),requestLimits:requestLimits(form)};try{await options.request('/api/admin/users',{method:'POST',body:JSON.stringify(payload)});setCreating(false);setCreateRole('user');options.notify('User created.');await load();}catch(reason){options.notify(message(reason),'error');}}
  async function saveAccess(event:FormEvent<HTMLFormElement>){event.preventDefault();if(!editing)return;const form=new FormData(event.currentTarget);try{await options.request(`/api/admin/users/${editing.id}`,{method:'PATCH',body:JSON.stringify({action:'permissions',permissions:selectedPermissions(form),requestApprovalRequired:form.has('requestApprovalRequired'),requestLimits:requestLimits(form)})});setEditing(null);options.notify('User access updated. Changes apply to active sessions immediately.');await load();}catch(reason){options.notify(message(reason),'error');}}
  ...
  if(error)return <RouteError title="Users unavailable" message={error}/>;
