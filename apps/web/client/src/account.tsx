...
function AccessFields({permissions,defaultChecked=false}:{permissions?:AccountUser['permissions'];defaultChecked?:boolean}){
  return <fieldset className="user-access-fields"><legend>Page access</legend><p>Choose the pages this user can open. Movie, TV, Dashboard, and Calendar access is view-only; Discover can add titles.</p>{pagePermissions.map(([key,label,help])=><label className="user-access-option" key={key}><input type="checkbox" name={`permission_${key}`} defaultChecked={permissions?.[key]??defaultChecked}/><span><strong>{label}</strong><small>{help}</small></span></label>)}</fieldset>;
}
...
function Users({options}:{options:AccountMountOptions}){
  ...
  async function create(event:FormEvent<HTMLFormElement>){event.preventDefault();const form=new FormData(event.currentTarget),payload={...Object.fromEntries(form.entries()),permissions:selectedPermissions(form)};try{await options.request('/api/admin/users',{method:'POST',body:JSON.stringify(payload)});setCreating(false);setCreateRole('user');options.notify('User created.');await load();}catch(reason){options.notify(message(reason),'error');}}
  async function saveAccess(event:FormEvent<HTMLFormElement>){event.preventDefault();if(!editing)return;const form=new FormData(event.currentTarget);try{await options.request(`/api/admin/users/${editing.id}`,{method:'PATCH',body:JSON.stringify({action:'permissions',permissions:selectedPermissions(form)})});setEditing(null);options.notify('User access updated. Changes apply to active sessions immediately.');await load();}catch(reason){options.notify(message(reason),'error');}}
