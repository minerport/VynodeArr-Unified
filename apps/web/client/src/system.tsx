...
import type { ApplicationUpdate,DiskSpace,SystemDomain,SystemMountOptions,SystemRecord,SystemView } from './system-types';
...
export function SystemView({options}:{options:SystemMountOptions}){
  ...
  return <div className="react-system"><div className="hero"><div><span className="eyebrow">SYSTEM</span><h1>System</h1><p className="lede">Storage, scheduled tasks, backups, updates, and engine events.</p></div><button className="primary" disabled={loading} onClick={()=>void sync()}>Synchronize now</button></div><div className="settings-tabs system-tabs">{(['status','tasks','backups','updates','events'] as SystemView[]).map(item=><button className={view===item?'wanted-tab active':'wanted-tab'} onClick={()=>setView(item)} key={item}>{item}</button>)}</div>{loading?<div className="panel skeleton">Loading system information…</div>:error?<div className="panel error-state"><h2>System information unavailable</h2><p>{error}</p><button className="secondary" onClick={()=>void load()}>Try again</button></div>:view==='status'?<Status disks={disks}/>:view==='tasks'?<Tasks items={records} run={item=>void run(item)}/>:view==='backups'?<Backups items={records} options={options} reload={()=>void load()}/>:view==='events'?<Events items={records}/>:update?<div className="panel"><div className="application-update"><div><span className="eyebrow">VYNODEARR APPLICATION</span><h2>Version {update.installedVersion}</h2><p className="muted">{update.channel} · {update.mechanism}</p></div><span className="badge green">Installed</span></div><div className="notice"><strong>VynodeArr updates</strong><p>{update.message}</p><a className="secondary button-link" href={update.repository} target="_blank" rel="noreferrer">View releases</a></div></div>:null}</div>;
}
