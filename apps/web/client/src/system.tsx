...
import type { ApplicationUpdate,DiskSpace,MasterKeyStatus,SystemDomain,SystemMountOptions,SystemRecord,SystemView } from './system-types';
...
function Backups({items,options,reload}:{items:SystemRecord[];options:SystemMountOptions;reload:()=>void}){
  ...
  return <><div className="system-toolbar"><div className="notice"><strong>Keep copies outside the container</strong><p>Download backups before removing application data or Docker volumes.</p></div><button className="primary" onClick={()=>void create()}>Create both backups</button></div><div className="system-domain-grid">{domains.map(domain=>{const records=items.filter(item=>item.domain===domain);return <DomainSection domain={domain} count={records.length} key={domain}><input ref={node=>{inputs.current[domain]=node;}} hidden type="file" accept=".zip,.db,.xml" onChange={event=>{void upload(domain,event.target.files?.[0]);event.target.value='';}}/><button className="secondary system-upload" onClick={()=>inputs.current[domain]?.click()}>Upload & restore</button>{records.map(item=><div className="data-row" key={`${domain}:${item.id}`}><span><strong>{label(domain)} configuration backup</strong><small>{date(item.time)} · {item.type||'manual'} · {size(item.size)}</small></span><div><a className="secondary button-link" href={`/api/system/backups/${domain}/${item.id}/download`}>Download</a><button className="secondary" onClick={()=>void restore(item)}>Restore</button></div></div>)}</DomainSection>;})}</div></>;
}
...
export function SystemView({options}:{options:SystemMountOptions}){
  ...
  return <div className="react-system"><div className="hero"><div><span className="eyebrow">SYSTEM</span><h1>System</h1><p className="lede">Storage, scheduled tasks, backups, updates, security, and engine events.</p></div><button className="primary" disabled={loading} onClick={()=>void sync()}>Synchronize now</button></div><div className="settings-tabs system-tabs">{(['status','tasks','backups','updates','security','events'] as SystemView[]).map(item=><button className={view===item?'wanted-tab active':'wanted-tab'} onClick={()=>setView(item)} key={item}>{item}</button>)}</div>{loading?<div className="panel skeleton">Loading system information…</div>:error?<div className="panel error-state"><h2>System information unavailable</h2><p>{error}</p><button className="secondary" onClick={()=>void load()}>Try again</button></div>:view==='status'?<Status disks={disks}/>:view==='tasks'?<Tasks items={records} run={item=>void run(item)}/>:view==='backups'?<Backups items={records} options={options} reload={()=>void load()}/>:view==='events'?<Events items={records}/>:view==='security'&&masterKey?<Security status={masterKey} options={options} reload={()=>void load()}/>:update?<div className="panel"><div className="application-update"><div><span className="eyebrow">VYNODEARR APPLICATION</span><h2>Version {update.installedVersion}</h2><p className="muted">{update.channel} · {update.mechanism}</p></div><span className="badge green">Installed</span></div><div className="notice"><strong>VynodeArr updates</strong><p>{update.message}</p><a className="secondary button-link" href={update.repository} target="_blank" rel="noreferrer">View releases</a></div></div>:null}</div>;
}
