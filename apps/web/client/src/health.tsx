...
export function HealthView({options}:{options:HealthMountOptions}){
  ...
  const [items,setItems]=useState<HealthItem[]>([]),[engineInstanceId,setEngineInstanceId]=useState('all'),[loading,setLoading]=useState(true),[error,setError]=useState(''),[busy,setBusy]=useState(''),[verifications,setVerifications]=useState<Record<string,EnginePathVerification>>({});
  const load=useCallback(async(refresh=false)=>{setLoading(true);try{const value=await options.request<{items?:HealthItem[]}>(`/api/system/health${refresh?'?refresh=1':''}`);setItems(value.items||[]);setError('');}catch(reason){setError(errorMessage(reason,'Health checks could not be loaded.'));}finally{setLoading(false);}},[options]);
  useEffect(()=>{void load();},[load]);
  const dismiss=async(item:HealthItem)=>{setBusy(item.id);try{await options.request(`/api/system/health/${encodeURIComponent(item.id)}/dismiss`,{method:'POST'});setItems(current=>current.filter(value=>value.id!==item.id));}catch(reason){setError(errorMessage(reason,'The warning could not be dismissed.'));}finally{setBusy('');}};
  const rematch=async(item:HealthItem)=>{const recovery=item.recovery;if(!recovery?.libraryItem||!recovery.replacement)return;if(!window.confirm(`Update ${recovery.libraryItem.title} to ${recovery.replacement.title}${recovery.replacement.year?` (${recovery.replacement.year})`:''}? Existing files and library settings will be preserved.`))return;setBusy(item.id);try{await options.request(`/api/system/health/${encodeURIComponent(item.id)}/rematch`,{method:'POST',body:JSON.stringify({mediaId:recovery.libraryItem.id,tmdbId:recovery.replacement.tmdbId,engineInstanceId:item.engineInstanceId})});setItems(current=>current.filter(value=>value.id!==item.id));}catch(reason){setError(errorMessage(reason,'The library match could not be updated.'));}finally{setBusy('');}};
  const verify=async(item:HealthItem)=>{const path=warningPath(item);if(!path)return;setBusy(item.id);try{const value=await options.request<EnginePathVerification>(`/api/storage/engine-path-verification?domain=${item.domain}&path=${encodeURIComponent(path)}${item.engineInstanceId?`&engineInstanceId=${encodeURIComponent(item.engineInstanceId)}`:''}`);setVerifications(current=>({...current,[item.id]:value}));}catch(reason){setError(errorMessage(reason,'The engine mapping could not be verified.'));}finally{setBusy('');}};
  const remap=async(item:HealthItem,target:string)=>{const sourceRoot=warningPath(item),verification=verifications[item.id];if(!sourceRoot||!verification?.equivalentTargets.includes(target))return;const total=verification.titleCount+verification.collectionCount;if(!window.confirm(`Update ${total} stored ${item.domain==='movie'?'movie/collection':'television'} path${total===1?'':'s'} from ${sourceRoot} to ${target}? No files will be moved.`))return;setBusy(item.id);try{await options.request('/api/storage/engine-path-remap',{method:'POST',body:JSON.stringify({domain:item.domain,sourceRoot,targetRoot:target,engineInstanceId:item.engineInstanceId})});const value=await options.request<EnginePathVerification>(`/api/storage/engine-path-verification?domain=${item.domain}&path=${encodeURIComponent(sourceRoot)}${item.engineInstanceId?`&engineInstanceId=${encodeURIComponent(item.engineInstanceId)}`:''}`);setVerifications(current=>({...current,[item.id]:value}));}catch(reason){setError(errorMessage(reason,'The engine paths could not be remapped.'));}finally{setBusy('');}};
  const visibleItems=engineInstanceId==='all'?items:items.filter(item=>item.engineInstanceId===engineInstanceId);
  return <div className="react-health"><div className="hero"><div><span className="eyebrow">{options.setupMode?'INSTALLATION SETUP':'SYSTEM HEALTH'}</span><h1>{options.setupMode?'Final health check':'Health'}</h1><p className="lede">Check engine connections, providers, storage, and import paths. Every reported issue links to the setting needed to resolve it.</p></div><button className="secondary" disabled={loading} onClick={()=>void load(true)}>{loading?'Checking engines…':'Check health now'}</button></div>{options.setupMode?<SetupNav active="health"/>:null}
    <div className="settings-context-bar"><EngineInstanceFilter instances={engineInstances} value={engineInstanceId} onChange={setEngineInstanceId}/></div>
    ...
    {loading&&!items.length?<div className="system-domain-grid health-domain-grid skeleton">Loading health checks…</div>:<div className="system-domain-grid health-domain-grid"><DomainHealth domain="movie" items={visibleItems.filter(item=>item.domain==='movie')} busy={busy} verifications={verifications} onDismiss={dismiss} onRematch={rematch} onVerify={verify} onRemap={remap}/><DomainHealth domain="tv" items={visibleItems.filter(item=>item.domain==='tv')} busy={busy} verifications={verifications} onDismiss={dismiss} onRematch={rematch} onVerify={verify} onRemap={remap}/></div>}
  </div>;
}
