...
export function LibraryView({options}:{options:LibraryMountOptions}){
  ...
  const [items,setItems]=useState(options.items),[filter,setFilter]=useState('all'),[sort,setSort]=useState('title'),[query,setQuery]=useState(''),[view,setView]=useState(options.initialView),[limit,setLimit]=useState(240),[searching,setSearching]=useState(false);
  const visible=useMemo(()=>items.filter(item=>item.title.toLowerCase().includes(query.toLowerCase())).filter(item=>filter==='all'||filter==='monitored'&&item.monitoring!=='none'||filter==='missing'&&item.monitoring!=='none'&&(movie?item.state==='missing':Number(item.missingEpisodes||0)>0)||filter==='cutoff'&&movie&&item.monitoring!=='none'&&item.state==='cutoff').sort((a,b)=>sort==='year'?Number(b.year||0)-Number(a.year||0):sort==='attention'?(movie?Number(a.state==='available')-Number(b.state==='available'):Number(b.missingEpisodes||0)-Number(a.missingEpisodes||0)):a.title.localeCompare(b.title)),[items,filter,sort,query,movie]);
