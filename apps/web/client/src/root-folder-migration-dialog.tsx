    ...
    <ModalPortal>
      <div className="root-folder-browser-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget && !busy) onClose(); }}>
        <section className="panel root-folder-browser" role="dialog" aria-modal="true" aria-label="Existing library path detected">
          ...
          <div className="migration-summary"><div><strong>{match.affected.length}</strong><small>existing {domain === "movie" ? "movies" : "series"} use the old path</small></div>{match.affectedCollections?.length ? <div><strong>{match.affectedCollections.length}</strong><small>movie collections use the old root</small></div> : null}<p>This updates the {domain === "movie" ? "movie" : "television"} engine first, immediately refreshes VynodeArr, and verifies that no titles still use the old path. Files are not copied, renamed, moved, or deleted. Keep both container mappings until verification finishes.</p></div>
          <details>
