...
export function ManagementView({options}:{options:ManagementMountOptions}){
  ...
  const loadCatalog=useCallback(async()=>{
    setLoading(true);setError('');try{const value=await options.request<{resources:ManagementResource[];available:boolean}>(`/api/manage/${domain}`),items=value.resources||[];setResources(items);const next=items.some(item=>item.key===resource)?resource:items[0]?.key||'';setResource(next);if(!value.available||!next){setRecords([]);setLoading(false);}}
    catch(reason){setError(message(reason));setLoading(false);}
  },[domain,resource,options]);
  useEffect(()=>{void loadCatalog();},[domain]);
  useEffect(()=>{if(resource)void loadRecords(resource);},[domain,resource]);
  ...
  const save=async()=>{if(!selected)return;const exists=selected.id!==undefined,method=exists?'PUT':methods.includes('POST')?'POST':methods.includes('PUT')?'PUT':null;if(!method)return;const path=`/api/manage/${domain}/${resource}${exists?`/${selected.id}`:''}`;setBusy(true);try{await options.request(path,{method,body:JSON.stringify(selected)});options.notify(exists||method==='PUT'?'Changes saved.':'Record created.');await loadRecords();}catch(reason){options.notify(message(reason),'error');}finally{setBusy(false);}};
