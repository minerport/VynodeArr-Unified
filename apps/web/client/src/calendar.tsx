...
export function CalendarView({options}:{options:CalendarMountOptions}){
  ...
  return <div className="react-calendar">
    ...
    <div className="calendar-layout">
      {loading?<div className="calendar-shell skeleton">Loading…</div>:error?<div className="calendar-shell"><div className="empty error-state"><h2>Calendar unavailable</h2><p>{error}</p></div></div>:<div className="calendar-shell"><div className="calendar-weekdays">{['Sun','Mon','Tue','Wed','Thu','Fri','Sat'].map(day=><span key={day}>{day}</span>)}</div><div className="calendar-grid">{cells.map((cell,index)=>cell?<button type="button" key={cell.key} className={`calendar-day${cell.key===dateKey(today)?' today':''}${cell.key===selected?' selected':''}`} onClick={()=>setSelected(cell.key)}><span className="calendar-date">{cell.day}</span>{cell.items.length?<span className="calendar-count">{cell.items.length}</span>:null}<span className="calendar-day-events">{cell.items.slice(0,3).map((event,eventIndex)=><span className={`calendar-day-event ${event.domain}`} key={`${event.domain}:${event.id}:${eventIndex}`}>{eventTitle(event)}</span>)}{cell.items.length>3?<small>+{cell.items.length-3} more</small>:null}</span></button>:<span className="calendar-day outside" aria-hidden="true" key={index}/>)}</div></div>}
