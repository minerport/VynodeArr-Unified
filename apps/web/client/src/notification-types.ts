...
export interface DownloadDecision{ id:string;domain:'movie'|'tv';mediaId:number|null;source:'interactive'|'automatic'|'engine';title:string;indexer:string;protocol:string;decision:'selected'|'accepted'|'rejected';reasons:string[];quality:string;customFormatScore:number;preferredWordScore:number;size:number;ageDays:number|null;seeders:number|null;upgradeEligible:boolean|null;previousQuality?:string|null;previousCustomFormatScore?:number|null;currentQuality?:string|null;currentCustomFormatScore?:number|null;engineEventType?:string|null;observedAt:string;selectedAt:string|null;requesters?:Array<{id:string;name:string;username:string;requestedAt?:string|null}>; }
