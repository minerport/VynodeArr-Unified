...
export function EngineManagementView({options}:{options:EngineManagementMountOptions}){
  ...
  const selectMode=async(next:'bundled'|'external')=>{if(next==='external'&&!confirm('Switch to your external movie and television engines after the next container restart? Bundled engines will remain stopped while external mode is active.'))return;if(next==='bundled'&&!confirm('Return to the bundled engines after the next container restart?'))return;try{await options.request('/api/settings/engines/mode',{method:'PUT',body:JSON.stringify({mode:next})});options.notify('Engine mode saved. Restart the VynodeArr container to apply it.','info');await load();}catch(reason){options.notify(errorText(reason),'error');}};
  ...
  return <div className="react-engine-management">
    ...
    <details className="external-engine-settings" open={system.engineMode==='external'||system.pendingMode==='external'}><summary><span><strong>Advanced: external engines</strong><small>Connect your movie and television engines, then validate both before activating external mode.</small></span></summary><div className="engine-wizard"><ExternalEngineForm domain="movie" initial={settings.external.movie} options={options} onSaved={()=>void load()}/><ExternalEngineForm domain="tv" initial={settings.external.tv} options={options} onSaved={()=>void load()}/></div></details>
  </div>;
}
