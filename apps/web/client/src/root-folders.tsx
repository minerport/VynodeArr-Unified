...
export function RootFoldersView({ options }: { options: RootFoldersMountOptions }) {
  ...
  const registerAvailableFolder = async (path: string, targetDomain: StorageDomain | "music" = domain) => {
    ...
    try {
      ...
      options.notify(targetDomain==="music"?`${path} is now the Music library root.`:`${path} added to the ${targetDomain === "movie" ? "movie" : "television"} engine. Configure its destination below.`);
      await load();
    } catch (reason) {
      options.notify(message(reason), "error");
    } finally {
  ...
  return (
    <div className="root-folders-react-route">
      ...
      <section className="panel available-library-mappings">
        ...
        <div className="main-media-folders"><div className="panel-heading"><div><h3>Main media folder</h3><p className="muted">{availableFolders?.mainMediaConfigured?'Each row is one direct child of /media. Choose the folder itself, or show its subfolders to select a more specific library location.':'Map your shared host media folder to /media in the VynodeArr container, apply the change, and return here.'}</p></div><span className="badge">/media</span></div>{availableFolders?.mainMediaConfigured?(mediaChildren.length?<div className="available-mapping-list">{mediaChildren.map(item=>{const assigned=item.registeredMovie?'Movies':item.registeredTv?'Television':item.registeredMusic?'Music':null,open=expandedMediaPath===item.path;return <div className="media-folder-tree-node" key={item.path}><article className="storage-path-row"><span className={`storage-path-state ${assigned?'available':'pending'}`}/><div className="storage-path-copy"><strong>{item.label}</strong><small>{item.path}</small><small>{assigned?`Assigned to ${assigned}${assigned==='Music'?'.':' and registered with that engine.'}`:'Choose which engine should manage this folder, or browse its next level.'}</small></div><div className="storage-path-actions"><button className="secondary" disabled={Boolean(busy)} onClick={()=>void toggleMediaFolder(item.path)}>{open?'Hide subfolders':'Show subfolders'}</button>{!assigned?<div className="button-row"><button className="secondary" disabled={Boolean(busy)} onClick={()=>void registerAvailableFolder(item.path,'movie')}>Use for Movies</button><button className="secondary" disabled={Boolean(busy)} onClick={()=>void registerAvailableFolder(item.path,'tv')}>Use for Television</button><button className="secondary" disabled={Boolean(busy)} onClick={()=>void registerAvailableFolder(item.path,'music')}>Use for Music</button></div>:null}</div></article>{open&&FolderChildren?<FolderChildren busy={busy} parentPath={item.path} options={options} onRegister={registerAvailableFolder}/>:null}</div>;})}</div>:<div className="empty compact"><p>No direct child folders were found beneath /media. Create the library folders on the host, then refresh this page.</p></div>):null}</div>
