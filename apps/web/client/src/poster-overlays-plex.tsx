...
export function PlexConnectionPanel({
  ...
  const restore = async (application: PlexPosterApplication) => {
    setBusy(true);
    try {
      if(application.source==="list"&&application.listId&&application.restoreKind){
        await options.request(`/api/reeltrack/imported-lists/${encodeURIComponent(application.listId)}/artwork/${application.restoreKind}/restore`,{method:"POST",body:"{}"});
        await load();
        options.notify(`${application.title} ${application.restoreKind==="collection"?"collection poster":"title artwork"} was restored.`);
      }else{
        const value=await options.request<{application:PlexPosterApplication}>(`/api/poster-overlays/plex/applications/${application.id}/restore`,{method:"POST",body:"{}"});
        setApplications(current=>current.map(item=>item.id===application.id?value.application:item));
        options.notify(`${application.title} was restored to its captured Plex poster.`);
      }
    } catch (reason) {
      options.notify(errorText(reason), "error");
    } finally {
  ...
  const restoreMany=async(items:PlexPosterApplication[])=>{
    ...
    try{
      const unique=[...new Map(items.map(item=>[item.source==="list"?`list:${item.listId}:${item.restoreKind}`:item.id,item])).values()];
      for(const application of unique){
        try{
          if(application.source==="list"&&application.listId&&application.restoreKind)await options.request(`/api/reeltrack/imported-lists/${encodeURIComponent(application.listId)}/artwork/${application.restoreKind}/restore`,{method:"POST",body:"{}"});
          else await options.request(`/api/poster-overlays/plex/applications/${application.id}/restore`,{method:"POST",body:"{}"});
          restored++;
        }catch(reason){options.notify(`${application.title}: ${errorText(reason)}`,"error");}
      }
      await load();
      options.notify(`${restored} artwork restoration${restored===1?"":"s"} completed.`);
    }finally{setBusy(false);}
  };
  const restorableApplications=applications.filter(item=>item.status==="applied"&&item.restorable!==false),filteredRestores=restorableApplications.filter(item=>matchesFilters(item.variableValues,variableFilters));
  return (
    <section
    ...
    >
      ...
      <style>{`.plex-history-panel{padding:0!important;overflow:hidden}.plex-history-summary{display:flex;align-items:center;justify-content:space-between;gap:12px;padding:12px 14px;cursor:pointer;list-style:none}.plex-history-summary::-webkit-details-marker{display:none}.plex-history-summary h3,.plex-history-summary p{margin:0}.plex-history-summary .secondary{padding:.45rem .65rem}.plex-history-panel[open] .plex-history-summary{border-bottom:1px solid var(--border)}.plex-history-body{display:grid;gap:8px;padding:10px 14px}.plex-history-body>p{margin:0}.plex-history-header{justify-content:flex-end}.plex-history-list{display:grid;grid-template-columns:1fr!important;gap:0;margin:0}.plex-history-card{display:grid;grid-template-columns:minmax(0,1fr) auto!important;min-height:0!important;padding:6px 2px;border:0;border-bottom:1px solid var(--border);border-radius:0;background:transparent}.plex-history-content{display:flex;align-items:center;gap:7px;min-width:0;white-space:nowrap}.plex-history-content strong,.plex-history-content small{overflow:hidden;text-overflow:ellipsis}.plex-history-content small{color:var(--muted)}.plex-history-content .badge{flex:0 0 auto}.plex-history-card .form-actions button{width:auto!important;padding:.35rem .55rem}@media(max-width:760px){.plex-history-summary{align-items:flex-start}.plex-history-card{grid-template-columns:1fr!important}.plex-history-content{display:grid;grid-template-columns:minmax(0,1fr) auto;white-space:normal}.plex-history-content small{grid-column:1}.plex-history-content .badge{grid-column:2;grid-row:1/3}.plex-history-card .form-actions button{width:100%!important}}`}</style>
          ...
          >
            <figure style={{ margin: 0 }}>
              <img
                ...
                src={previewUrl("current", selectedEntry)}
                alt={`Current Plex poster for ${selectedEntry.title}`}
              />
              <figcaption>Current Plex poster · all managed overlays</figcaption>
            </figure>
            <figure style={{ margin: 0 }}>
              <img
                style={{
              ...
              />
              <figcaption>Exact result after this overlay</figcaption>
            </figure>
          </div>
          <button
            type="button"
              ...
              (application) => (
                <article className="plex-history-card" key={application.id}>
                  <div className="plex-history-content">
                    ...
                    <small>{application.source==="list"?`List · ${({collection:"collection poster",placeholder:"placeholder overlay",existing:"existing-title overlay",newly_available:"newly-downloaded overlay",title_overlays:"title overlays"} as Record<string,string>)[application.role||""]||"artwork"}`:"Plex overlay"} · {application.templateName} · {application.plexLibraryTitle}{application.affectedCount&&application.affectedCount>1?` · ${application.affectedCount} posters`:""} · {new Date(application.appliedAt).toLocaleString()}</small>
                    <span
