...
export function DiscoverRequest({item,options,onClose,onRequested}:{item:DiscoverItem;options:DiscoverMountOptions;onClose:()=>void;onRequested:(item:DiscoverItem)=>void}){
  ...
  useEffect(()=>{
    ...
    const engineQuery=engineInstanceId?`&engineInstanceId=${encodeURIComponent(engineInstanceId)}`:'';
  ...
  const submit=async(event:FormEvent)=>{
    ...
    setSubmitting(true);
    try{
      const response=await options.request<{request?:{status?:string}}>('/api/discover/request',{method:'POST',body:JSON.stringify({domain:resolvedItem.domain,tmdbId:resolvedItem.tmdbId,engineInstanceId,payload:{...payload,engineInstanceId}})});
