...
export async function queueBackgroundImport(options:QueueBackgroundImportOptions):Promise<ImportJob>{
  const {job}=await options.request<{job:ImportJob}>('/api/import-jobs',{method:'POST',body:JSON.stringify({domain:options.domain,items:options.items,engineInstanceId:options.engineInstanceId||undefined})});
