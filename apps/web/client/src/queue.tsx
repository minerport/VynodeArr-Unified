...
const source=(item:QueueItem)=>item.domain==='movie'?'Movie engine':'Television engine';
...
export function QueueView({options}:{options:QueueMountOptions}){
  ...
  const counts=useMemo(()=>({
    ...
    television:items.filter(item=>item.domain==='tv').length,
  }),[items]);
  ...
  const remove=async(item:QueueItem,ask=true)=>{if(ask&&!window.confirm('Remove this queue item?'))return;const itemKey=key(item);setBusy(old=>new Set(old).add(itemKey));try{await options.request(`/api/manage/${item.domain}/queue/${item.id}?removeFromClient=true&blocklist=false`,{method:'DELETE'});setItems(old=>old.filter(value=>key(value)!==itemKey));setSelected(old=>{const next=new Set(old);next.delete(itemKey);return next;});options.notify('Queue item removed.');await load();}catch(reason){options.notify(reason instanceof Error?reason.message:'Queue item could not be removed.','error');}finally{setBusy(old=>{const next=new Set(old);next.delete(itemKey);return next;});}};
  ...
  const removeSelected=async()=>{
    ...
    try{
      const result=await options.request<{removed:Array<{domain:string;id:string|number}>;failed:Array<{domain:string;id:string|number;message:string}>;items:QueueItem[]}>('/api/manage/queue/bulk-delete',{method:'POST',body:JSON.stringify({items:targets.map(({domain,id})=>({domain,id})),removeFromClient:true,blocklist:false})});
