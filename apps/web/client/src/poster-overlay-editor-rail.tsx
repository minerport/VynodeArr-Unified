...
export default function EditorRail({ editing, request, notify, selectedId, selectedIds, query, onQuery, onSelect, onDuplicate, onMove, onChange, onAddText, onAddIcon, onAddShape, onAddPreset }: Props) {
  ...
  const add=(asset:OverlayAsset)=>{const layer=overlayLayerFromAsset(asset);onChange({layers:[...editing.layers,layer]});onSelect(layer.id);},remove=async(asset:OverlayAsset)=>{try{await request(`/api/poster-overlays/assets/${asset.id}`,{method:"DELETE"});setAssets(current=>current.filter(item=>item.id!==asset.id));notify(`${asset.name} deleted.`);}catch(reason){notify(errorMessage(reason,"The image could not be deleted."),"error");}},upload=async(file:File)=>{setUploading(true);try{const image=await new Promise<string>((resolve,reject)=>{const reader=new FileReader();reader.onload=()=>resolve(String(reader.result));reader.onerror=()=>reject(new Error("The image could not be read."));reader.readAsDataURL(file);}),value=await request<{asset:OverlayAsset}>("/api/poster-overlays/assets",{method:"POST",body:JSON.stringify({name:file.name.replace(/\.[^.]+$/,"")||"Overlay image",image})});setAssets(current=>[...current,value.asset]);add(value.asset);notify(`${value.asset.name} uploaded and added.`);}catch(reason){notify(errorMessage(reason,"The image could not be uploaded."),"error");}finally{setUploading(false);}};
  return (
