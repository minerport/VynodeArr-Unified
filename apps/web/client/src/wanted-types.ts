...
export interface WantedItem {
  id:number; seriesId?:number; seasonNumber?:number; episodeNumber?:number; title?:string; status?:string;
  movie?:{title?:string}; series?:{title?:string;tvdbId?:number}; qualityProfile?:{name?:string};
}
export interface WantedLibraryItem {id:number;title?:string;tvdbId?:number}
