...
function AddResult({item,index,domain,profiles,roots,destinations,options}:{item:AddMediaResult;index:number;domain:AddMediaDomain;profiles:AddMediaProfile[];roots:AddMediaRoot[];destinations:AddMediaDestination[];options:AddMediaMountOptions}){
  ...
  return <article className="discovery-card" data-result-index={index}>
    ...
    <form onSubmit={event=>void add(event)}>
      {destinations.length?<label>Library destination<select value={destinationId} onChange={event=>setDestinationId(event.target.value)}>{destinations.map(value=><option key={value.id} value={value.id} disabled={!value.ready}>{value.name}{value.isDefault?' — default':''}{value.ready?'':' — unavailable'}</option>)}</select><small>The selected folder and profile are sent to the media engine.</small></label>:null}
